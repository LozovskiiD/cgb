*log_prim - строка примечаний
*log_sr  - средний оклад
*log_kom - тарификационная комиссия
*log_it - итоги по категориям персонала после отделений
*avt_vac - вакансии автоматически
*vacst - вакансии не более 1 ставки
*log_vac - исключаит вакантные
logKom=.F. &&для печати комиссии
kpdop=0
margTop=000.00
nlspace=000.00
...
*dimOption(2) - подразделение
*dimOption(3) - персонал
*dimOption(4) - список
*dimOption(5) - тип работы
*dimOption(6) - должность
...
WITH fSupl
     ...
     DO adTboxAsCont WITH 'fSupl','txtVed',.Shape1.Left+10,.Shape1.Top+10,.Shape1.Width-RetTxtWidth('w99/99/9999w')-19,dHeight,'ведомость',2,1
     DO adTboxAsCont WITH 'fSupl','txtDate',.txtVed.Left+.txtVed.Width-1,.txtVed.Top,RetTxtWidth('w99/99/9999w'),dHeight,'дата',2,1
     ...
     WITH .comboBox1         && ведомости   
          .nDisplayCount=18
     ENDWITH
     DO adTboxNew WITH 'fSupl','boxDate',.comboBox1.Top,.txtDate.Left,.txtDate.Width,dHeight,'dateTar',.F.,.T.,0
     DO adTboxAsCont WITH 'fSupl','txtNad',.txtVed.Left,.comboBox1.Top+.comboBox1.Height-1,.txtVed.Width+.txtDate.Width-1,dHeight,'надбавка/доплата',2,1
     ...
     DO adTboxAsCont WITH 'fSupl','txtPeriod',.txtVed.Left,.comboBox2.Top+.comboBox2.Height-1,.txtNad.Width-.txtDate.Width*2+2,dHeight,'период',0,1
     ...
     DO adCheckBox WITH 'fSupl','checkGroup','Группа печати',.Shape2.Top+10,.Shape2.Left+5,150,dHeight,'dimOption(1)',0,.T.,'DO validCheckGroup'          
     DO adCheckBox WITH 'fSupl','checkPodr','Подразделение',.checkGroup.Top+.checkGroup.Height+10,.Shape2.Left+5,150,dHeight,'dimOption(2)',0,.T.,'DO validCheckPodr'  
             
     DO adCheckBox WITH 'fSupl','checkPers','Персонал',.checkGroup.Top,.Shape2.Left+5,150,dHeight,'dimOption(3)',0,.T.,'DO validCheckPers' 
     DO adCheckBox WITH 'fSupl','checkSpis','Список',.checkPodr.Top,.Shape2.Left+5,150,dHeight,'dimOption(4)',0,.T.,'DO validChecKSpis'
     
     DO adCheckBox WITH 'fSupl','checkType','Тип работы',.checkGroup.Top,.Shape2.Left+5,150,dHeight,'dimOption(5)',0,.T.,'DO validCheckType' 
     DO adCheckBox WITH 'fSupl','checkDol','Должность',.checkPodr.Top,.checkType.Left,150,dHeight,'dimOption(6)',0,.T.,'DO validCheckDol' 
     ...
     DO adlabMy WITH 'fSupl',16,' Дополнительно ',.Shape4.Top-10,.Shape1.Left+10,300,0,.T.,1
     *----------------CheckBox для печати итогов по отделениям в разрезе категорий персонала---------------------------------
     DO adCheckBox WITH 'fSupl','check1','итог по категориям',.Shape4.Top+10,.Shape4.Left+(.Shape4.Width-RetTxtWidth('WВакансия автоматическиW')*2-30)/2,200,dHeight,'formbase.log_it',0,.T.  
     *----------------CheckBox автоматической подстановки вакансий-----------------------------------------------------------     
     DO adCheckBox WITH 'fSupl','checkAvt','вакансии автоматически',.check1.Top+.check1.Height+10,.check1.Left,200,dHeight,'formbase.avt_vac',0,.T.   
     *----------------CheckBox для вакантных не более ставки-----------------------------------------------------------     
     DO adCheckBox WITH 'fSupl','checkSt','вакантная не более 1.00',.checkAvt.Top+.checkAvt.Height+10,.check1.Left,200,dHeight,'formbase.vacst',0,.T.  
     *----------------CheckBox для итогов по типу работы-----------------------------------------------------------     
     DO adCheckBox WITH 'fSupl','checkItr','итог по типу работы',.checkSt.Top+.checkSt.Height+10,.check1.Left,200,dHeight,'formbase.litr',0,.T.           
     *-----------------------------CheckBox для включения (выключения)вакансий в ведомости------------------------------------  
     DO adCheckBox WITH 'fSupl','check5','исключать вакантные',.check1.Top,.check1.Left+RetTxtWidth('WВакансии автоматическиW')+10,200,dHeight,'excludeVac',0,.T.         
     *-----------------------------CheckBox для только вакантные----------------------------------------------------------------------------  
     DO adCheckBox WITH 'fSupl','checkOnlyVac','только вакантные',.checkAvt.Top,.check5.Left,200,dHeight,'onlyVac',0,.T.  
     *-----------------------------CheckBox для включения (выключения)Тарификационной комисси в ведомости------------------------------------  
     DO adCheckBox WITH 'fSupl','check7','комиссия',.checkSt.Top,.check5.Left,200,dHeight,'formbase.log_kom',0,.T.           
     .Shape4.Height=dHeight*4+50   
        
         
     DO adSetupPrnToForm WITH .Shape2.Left,.Shape4.Top+.Shape4.Height+10,.Shape1.Width,.F.,.T.
     DO adTBoxAsCont WITH 'fSupl','txtMarg',.Shape91.Left,.Shape91.Top+.Shape91.Height+10,RetTxtWidth('Wувеличить "шапку"'),dHeight,'увеличить "шапку"',2,1
     ...
     DO adTBoxAsCont WITH 'fSupl','txtSpace',.Shape91.Left,.txtMarg.Top,RetTxtWidth('Wмежстрочный интервал'),dHeight,'межстрочный интервал',2,1
...
WITH fSupl
     ...
     .checkPers.Caption='Персонал'+IIF(kvoKat#0,'('+LTRIM(STR(kvoKat))+')','') 
     .Refresh
ENDWITH 
*************************************************************************************************************************
...
WITH fSupl
     ...
     .checkSpis.Caption='Список'+IIF(kvoSpis#0,'('+LTRIM(STR(kvoSpis))+')','') 
     .Refresh
ENDWITH 
*************************************************************************************************************************
...
ENDIF
DO CASE
   CASE dimOption(1)       && группа  печати         
        ...
        ENDCASE               
   CASE dimOption(2)  && подразделение   
        fltpodr=fltpodr+',999,'   
        DO CASE        
           CASE !EMPTY(parField)
...
ENDIF
*************************************************************************************************************************
*   Вспомогательная процедура для формировапния курсора (currasp) и упорядочения сведений в people
*************************************************************************************************************************
...
SELECT sprkat
* maxkat=кол-во категорий персонала
* max_Tr=кол-во типов работы
* sum_podr - итог по отделениям 
* sumPodrKat - итог по отделениям в разрезе персонала

* sumTot - итог общий
* sumTotTr -итог общий по типам работы
* sumTotKat - итог общий по категориям персонала
* sumKpp -итог по подподразделению
...
STORE 0 TO sumTotTr  &&итоги по типу работы общие

DIMENSION sumPodrKpp(maxItog),sumPodrKpp1(maxItog)  &&итоги в подподразделениях и подпод подразделениях
STORE 0 TO sumPodrKpp,sumPodrKpp1

DIMENSION sumPodrKatKpp(maxKat,maxItog),sumPodrKatKpp1(maxKat,maxItog)  &&итоги в подподразделениях и подпод подразделениях по категориям персонала
STORE 0 TO sumPodrKatKpp,sumPodrKatKpp1

*массивы для итогов типу работы в разрезе категорий персонала
FOR i=1 TO maxKat
...
IF datShtat.Real
   SELECT curTarJob  
   DELETE FOR tr=4                  && удаляем записи по совмещению tr=4
   DELETE FOR kd=0.OR.kp=0
   DELETE FOR SEEK(kodpeop,'people',1).AND.people.dekotp.AND.people.bdekotp<=dateTar && удаляем декретчиков   
   ...
   IF parType#4 
      DELETE FOR SEEK(kodpeop,'people',1).AND.!EMPTY(people.date_out).AND.people.date_out<dateTar    && удаляем записи где дата увольнения меньще даты тарификации   
   ENDIF     
ENDIF 
*DELETE FOR !SEEK(STR(kp,3)+STR(kd,3),'rasp',2)

*   DELETE FOR SEEK(peopledate_in>newDateTar    && удаляем записи где дата приема больше даты тарификации
...
DO CASE   
   CASE parType=4
   OTHERWISE 
        DELETE FOR !EMPTY(dateOut).AND.EMPTY(dateuv).AND.dateOut<=dateTar    && удаляем записи где дата окончания<=работы меньще даты тарификации при пустой дате увольнения
        DELETE FOR !EMPTY(dateuv).AND.dateuv<dateTar  &&дата увольнения<даты тарификации при заполненной дате увольнения
ENDCASE  

IF parType#4
   DELETE FOR date_in>dateTar && удаляем прищедших позже даты т-ции
ELSE 
   DELETE FOR !INLIST(tr,1,3)
   REPLACE dateuv WITH IIF(SEEK(kodpeop,'people',1),people.date_out,dateuv) ALL
   DELETE FOR dateBeg<begTar    && удаляем записи где дата начала работы больше даты тарификации
   DELETE FOR dateBeg>endTar    && удаляем записи где дата начала работы больше даты тарификации 
   DELETE FOR !EMPTY(dateOut).AND.EMPTY(dateuv)   && удаляем записи где дата окончания<=работы меньще даты тарификации при пустой дате увольнения
   DELETE FOR !EMPTY(dateOut).AND.!EMPTY(dateuv).AND.dateuv<endTar   && удаляем записи где дата окончания<=работы меньще даты тарификации при пустой дате увольнения   
ENDIF    
      ...
      IF rasp.kse#0
         ...
         IF kse_cx>0
            IF !formbase.vacst
               SELECT curTarJob
               APPEND BLANK
               REPLACE kp WITH rasp.kp,kd WITH rasp.kd,kat WITH rasp.kat,fio WITH 'Вакантная', kv WITH rasp.kv,kf WITH rasp.kfvac,nameKf WITH rasp.nKfVac,pkf WITH rasp.pkf,kse WITH kse_cx,vac WITH .T.,tr WITH 1
               SELECT curPrnTarFond
               GO TOP
               DO WHILE !EOF()
            ...
            ELSE 
               ...
               FOR i=1 TO kvovac
                   ...
                   SELECT curTarJob
                   APPEND BLANK
                   REPLACE kp WITH rasp.kp,kd WITH rasp.kd,kat WITH rasp.kat,fio WITH 'Вакантная', kv WITH rasp.kv,kf WITH rasp.kfvac,nameKf WITH rasp.nKfVac,pkf WITH rasp.pkf,kse WITH ksevac,vac WITH .T.,tr WITH 1
                   SELECT curPrnTarFond
                   GO TOP
                   DO WHILE !EOF()                   
                      rep_r=ALLTRIM(persved)
...
IF excludeVac
   ...
   DELETE FOR vac
ENDIF
*---Курсор для штатного расписания
IF USED('curRasp')
   SELECT curRasp
   USE
ENDIF 
...
DO WHILE !EOF()     
   ...
   IF kp_cx#curTarJob.kp   
      ...
      SELECT curPrn
      APPEND BLANK      
      REPLACE nIt WITH 1,kp WITH kp_cx,kd WITH 999,fio WITH 'Итого',nd WITH 700
      FOR i=1 TO maxItog
         repSum=nSum(i)	
         REPLACE &repSum WITH sum_podr(i)
      ENDFOR
      &&  итог по категориям песонала в отделении
      IF itkat
         FOR i=1 TO maxKat
             APPEND BLANK
      ...
      IF kpp1_cx#curTarJob.kodkpp1.AND.kpp1_cx#0.AND.partype#4
         &&  итог по категориям песонала в под-подподразделении
         SELECT curprn
         APPEND BLANK      
         REPLACE nIt WITH 3,kp WITH kp_cx,kd WITH 999,fio WITH 'Итого',nd WITH 900
         FOR i=1 TO maxItog
             repSum=nSum(i)	
      ...
      ENDIF         
     *--------
     &&  итог по категориям песонала в подподразделении.
     IF kpp_cx#curTarJob.kodkpp.AND.kpp_cx#0.AND.partype#4      
         SELECT curprn
         APPEND BLANK      
         REPLACE nIt WITH 5,kp WITH kp_cx,kd WITH 999,fio WITH 'Итого',nd WITH 800
         FOR i=1 TO maxItog
             repSum=nSum(i)	
...
SELECT curPrn
APPEND BLANK      
REPLACE nIt WITH 7,kp WITH kp_cx,kd WITH 998,fio WITH IIF(formbase.litr,'ВСЕГО','Всего'),nd WITH 900,np WITH 900
FOR i=1 TO maxItog
    repSum=nSum(i)	
...
*-----Общий итог по категориям персонала
SELECT curprn
FOR i=1 TO maxKat
    APPEND BLANK 
...
DELETE FOR kse=0 
*DO CASE
*   CASE parItog=1           &&итоговая
*        DELETE FOR nIt<7 
*   CASE parItog=2           && по отделениям
*        DELETE FOR nIt=0 
*ENDCASE
IF parType=2
   DELETE FOR nIt<7
...
IF logKom
   ...
   REPLACE np WITH 999,nIt WITH 9,kp WITH kp_cx
   APPEND BLANK
   REPLACE np WITH 999,nprik WITH 'Комиссия:',nIt WITH 9,kp WITH kp_cx  
   SELECT boss
   SCAN ALL
        SELECT curprn
        APPEND BLANK
...
DO CASE
   CASE parType=1
        DO procForPrintAndPreview WITH 'reptarnew','Тарификационный список'
   CASE parType=2
        DO procForPrintAndPreview WITH 'reptarnew','Тарификационный список'
   CASE parType=3
        DO tarifToExcelNew
...
WITH excelBook.Sheets(1)
     ...
     WITH objExcel.Selection
          ...
          .VerticalAlignment=1
          .WrapText=.T.
          .Value='Список окладов (ставок) по должностям(профессиям) работников'
          .Font.Name='Times New Roman'   
          .Font.Size=9
     ENDWITH                        
     rowcx=rowcx+1                          
     rowBeg=rowcx
     .Cells(rowcx,1).Value='№ п.п'   
     .Cells(rowcx,2).Value='наименование вида услуг, структурного подразделения,фамилия, имя отчество работника'  
     .Cells(rowcx,3).Value='наименование должности'
     .Cells(rowcx,4).Value='тарифный разряд'
     .Cells(rowcx,5).Value='тарифный коэффициент'
     .Cells(rowcx,6).Value='оклад'
     .Cells(rowcx,7).Value='объём работы'               
     .Cells(rowcx,8).Value='оклад с учётом объёма работы'     
     .Range(.Cells(rowcx,1),.Cells(rowcx,8)).Select  
     WITH objExcel.Selection
          .HorizontalAlignment=xlCenter
          .VerticalAlignment=1
...
DIMENSION sumPodrKpp(2),sumPodrKpp1(2)  &&итоги в подподразделениях и подпод подразделениях
STORE 0 TO sumPodrKpp,sumPodrKpp1

DIMENSION sumPodrKatKpp(maxKat,2),sumPodrKatKpp1(maxKat,2)  &&итоги в подподразделениях и подпод подразделениях по категориям персонала
STORE 0 TO sumPodrKatKpp,sumPodrKatKpp1
      ...
      IF rasp.kse#0
         ...
         IF kse_cx>0
            IF !formbase.vacst               
               SELECT curPrn
               APPEND BLANK
               REPLACE kp WITH rasp.kp,kd WITH rasp.kd,kat WITH rasp.kat,fio WITH 'Вакантная', kv WITH rasp.kv,kf WITH rasp.kfvac,nameKf WITH rasp.nkfvac,pkf WITH rasp.pkf,kse WITH kse_cx,vac WITH .T.,;
            ...
            ELSE 
               ...
               FOR i=1 TO kvovac
                   ...
                   SELECT curPrn
                   APPEND BLANK
                   REPLACE kp WITH rasp.kp,kd WITH rasp.kd,kat WITH rasp.kat,fio WITH 'Вакантная', kv WITH rasp.kv,kf WITH rasp.kfvac,nameKf WITH rasp.nkfvac,pkf WITH rasp.pkf,kse WITH ksevac,vac WITH .T.,;
                       np WITH rasp.np,nd WITH rasp.nd,vtime WITH rasp.vtime,tr WITH 1,pkont WITH rasp.pkont
                   tar_ok=0
                   tar_ok=varBaseSt*curPrn.namekf*IIF(pkf#0,pkf,1)
...
SCAN ALL
     ...
     IF sprncx#0
        APPEND BLANK
        REPLACE fio WITH 'Всего',kp WITH sprpodr.kod,np WITH sprpodr.np,nd WITH 500,sprn WITH sprncx,kse WITH ksecx,nIt WITH 1     
     ENDIF
     IF itkat
        FOR i=1 TO maxKat
            SELECT curPrn
...
IF sprncx#0
   APPEND BLANK
   REPLACE fio WITH IIF(formbase.lItr,'ИТОГО','Итого'),kp WITH 999,np WITH 999,nd WITH 500,sprn WITH sprncx,kse WITH ksecx,nIt WITH 9  
   IF formbase.litr
      FOR i=1 TO max_tr
...
IF EMPTY(nSupProc)
   DO procForPrintAndPreview WITH 'repunit','Тарификационный список'
ELSE    
   DO &nSupProc
ENDIF    
SELECT curPrn
...
WITH excelBook.Sheets(1)
     ...
     WITH objExcel.Selection
          ...
          .VerticalAlignment=1
          .WrapText=.T.
          .Value='по состоянию на '+DTOC(dateTar)+' г.'
          .Font.Name='Times New Roman'   
          .Font.Size=9
     ENDWITH     
     rowcx=rowcx+1  
     ...
     With objExcel.Selection
          ...
          .VerticalAlignment=1
          .WrapText=.T.
          .Value='Наименование вида услуг, структурного подразделения Фамилия Имя Отчество работника'
          .Font.Name='Times New Roman'   
          .Font.Size=9
     ENDWITH   
...
WITH excelBook.Sheets(1)
     ...
     WITH objExcel.Selection
          ...
          .VerticalAlignment=1
          .WrapText=.T.
          .Value='Расчёт расходов на выплату доплаты работникам по результатам аттестации рабочих мест'
          .Font.Name='Times New Roman'   
          .Font.Size=9
     ENDWITH  
     ...
     WITH objExcel.Selection
          ...
          .VerticalAlignment=1
          .WrapText=.T.
          .Value='на '+ALLTRIM(dim_month(MONTH(dateTar)))+' '+STR(YEAR(dateTar),4)+' г.'
          .Font.Name='Times New Roman'   
          .Font.Size=9
     ENDWITH   
...
DO WHILE !EOF()
   ...
   IF kp#kpOld
      SELECT curPrn
      APPEND BLANK
      REPLACE kp WITH kpOld,np WITH npOld,nd WITH 90,named WITH 'всего',kse WITH ksePodr,kse_tot WITH totPodr,kse_am WITH amPodr,;
...
SELECT curPrn
APPEND BLANK
REPLACE np WITH 900,nd WITH 900,named WITH 'Итого по организации',kse WITH kseTot,kse_tot WITH totTot,kse_stac WITH stacTot,kse_am WITH amTot,;
...
INDEX ON STR(np,3)+STR(nd,3) TAG T1
GO TOP
DO procForPrintAndPreview WITH 'shtatRasp','Штатное расписание'

*************************************************************************************************************************
*                 Процедура печати ведомостей 
*************************************************************************************************************************
PROCEDURE vedprn
PARAMETERS par1
terminal=par1
...
COUNT TO maxSum
maxSum=maxSum+2
** Наименование суммируемых полей
...
**Итог по категориям персонала в подразделениях
...
DIMENSION sumKatKpp(maxKat,2),sumKatKpp1(maxKat,2) && итоги по категориям персонала в подподподразделнниях и подподрахднлениях
...
DO WHILE !EOF()     
   ...
   IF itkat.AND.curPrn.kat#0       &&Итогои по категориям персонала в подразделениях,под и подпод
      FOR i=1 TO maxSum
          IF kat#0
   ...
   IF kp_cx#curTarJob.kp   
      ...
      SELECT curPrn
      APPEND BLANK      
      REPLACE nIt WITH 1,kp WITH kp_cx,kd WITH 999,fio WITH 'Итого'
      FOR i=1 TO maxSum
      ...
      IF kpp1_cx#curTarJob.kodkpp1.AND.kpp1_cx#0
         ...
         SELECT curprn
         APPEND BLANK      
         REPLACE nIt WITH 3,kp WITH kp_cx,kd WITH 999,fio WITH 'Итого',kse WITH sumKpp1(1,1),mTokl WITH sumKpp1(1,2)        
         IF itkat
            FOR i=1 TO maxKat
                APPEND BLANK 
     ...
     *--------
     ...
     IF kpp_cx#curTarJob.kodkpp.AND.kpp_cx#0      
         SELECT curprn
         APPEND BLANK      
         REPLACE nIt WITH 5,kp WITH kp_cx,kd WITH 999,fio WITH 'Итого',kse WITH sumKpp(1,1),mTokl WITH sumKpp(1,2)
         IF itkat
            FOR i=1 TO maxKat
                APPEND BLANK 
...
SELECT curPrn
APPEND BLANK      
REPLACE nIt WITH 7,kp WITH kp_cx,kd WITH 999,fio WITH 'Всего'
FOR i=1 TO maxSum
...
SCAN ALL  
     ...
     newnpp=newnpp+1
ENDSCAN      
DO procForPrintAndPreview WITH 'reptarif','Тарификационный список'
SELECT people
**************************************************************************************************************************
*                  Процедура расчета оклада по вакансиям
**************************************************************************************************************************
...
WITH excelBook.Sheets(1)
     ...
     .Columns(8).ColumnWidth=8 
      ...
      rowcx=1     
     .Cells(rowcx,3).Value='базовая ставка'  
     ...
     With objExcel.Selection
           ...
           .Font.Name='Times New Roman'   
           .Font.Size=9
           .Value='наименование вида услугн, структурного подразделения, фамилия, имя, отчество работника'  
     ENDWITH    
    ...
    .Range(.Cells(rowcx,7),.Cells(rowcx+3,7)).Select
     ...
     columncx=9
     **  добавляем кололнки по доплатам          
     SELECT curFondPrn
     GO TOP
     DO WHILE !EOF()
        DO CASE 
           CASE !EMPTY(persved).AND.!EMPTY(sumved)
                ...
                .cells(rowcx+3,columncx+1).Value='сумма'
                columncx=columncx+2
     ...
     DO WHILE !EOF()
        IF kp#kpold
           ...
           kpold=kp
        ENDIF                                                    
        .Cells(rowcx,1).Value=IIF(curprn.npp#0.AND.nIt=0,curprn.npp,'')                           && номер по порядку                    
        .Cells(rowcx,2).Value=curprn.fio                                                          && фио                    
        .Cells(rowcx,3).Value=IIF(SEEK(curprn.kd,'sprdolj',1),ALLTRIM(sprdolj.name),'')           && должность
                 
        .Cells(rowcx,4).Value=IIF(SEEK(curprn.kv,'sprkval',1),ALLTRIM(sprkval.name),'')           && категория                                   
                
        .Cells(rowcx,5).NumberFormat = "General"
        .Cells(rowcx,5).Value=IIF(nIt=0,LEFT(staj_Tar,2)+'_'+SUBSTR(staj_tar,4,2)+'_'+SUBSTR(staj_tar,7,2),'')  && стаж                
                                      
        .Cells(rowcx,6).Value=IIF(curprn.kf#0,curprn.kf,'')                                       && разряд
                
        .Cells(rowcx,7).Value=IIF(curprn.namekf#0,curprn.namekf,'')                               && кфт
        .Cells(rowcx,7).NumberFormat = "#####.00"                       
                
        .Cells(rowcx,8 ).Value=IIF(curprn.tokl#0,curprn.tokl,'')                                  && оклад                  
...
SELECT sprkat
* maxkat=кол-во категорий персонала
* sumPodrTot - итог по отделению
* sum_podr - итог по отделениям в разрезе персонала
* sumTot - итог общий
* sum_podr - итог по отделениям в разрезе персонала
COUNT TO maxKat
...
DO WHILE !EOF()     
   ...
   IF kp_cx#curTarJob.kp   
      ...
      SELECT curPrn
      APPEND BLANK      
      REPLACE nIt WITH 1,kp WITH kp_cx,kd WITH 999,fio WITH 'Итого',kse WITH sumPodrTot(1,1)                                                         
      IF formbase.log_it
         FOR i=1 TO maxKat
             APPEND BLANK 
...
SELECT curPrn
APPEND BLANK      
REPLACE nIt WITH 7,kp WITH kp_cx,kd WITH 999,fio WITH 'Всего',kse WITH sumTot(1,1)  
*-----Общий итог по категориям персонала
IF formbase.log_it
   FOR i=1 TO maxKat
       APPEND BLANK 
...
DELETE FOR kse=0
GO TOP 
DO procForPrintAndPreview WITH 'repspisoknew','Список сотрудников'
SELECT people 
*SET ORDER TO &ord_old
SELECT rasp

*----------------------------------------------------------------------------------------------------------------------------
*            Создание и настройка нового файла отчета для тарификации
*----------------------------------------------------------------------------------------------------------------------------
...
LOCATE FOR objtype=6.AND.comment='lheadv'                  &&  Координаты длинных вертикальных линий в шапке 

leftline=hpos
leftobj=hpos+104.67*2
SELECT reptarnew                                           &&первая линия шапки
LOCATE FOR comment='lheadh'
htop=vpos  

LOCATE FOR objtype=8.AND.comment='lab1'                   && Координаты высоких заголовков в шапке   
SCATTER TO dim_lab                  

LOCATE FOR objtype=8.AND.ALLTRIM(comment)='labsup'        && Координаты заголовков доплат и надбавок в шапке   
SCATTER TO dim_labsup                  
DELETE 

LOCATE FOR objtype=8.AND.comment='labpers'                && Координаты заголовков %, сумма в шапке   
SCATTER TO dim_labpers                  
DELETE 

LOCATE FOR objtype=6.AND.comment='line1'                  && Координаты высоких вертикальных линий кололнок   
SCATTER TO dim_line   

LOCATE FOR objtype=6.AND.comment='linesup'                 && Координаты вертикальных линий кололнок доплат и надбавок в шапке   
SCATTER TO dim_linesup                  
DELETE 

LOCATE FOR objtype=6.AND.comment='lhpers'                 && Координаты горизонтальных линий процент-сумма
SCATTER TO dim_lhpers                  
DELETE 

LOCATE FOR objtype=6.AND.comment='lvpers'                 && Координаты вертикальных линий процент-сумма
SCATTER TO dim_lvpers                  
DELETE 

LOCATE FOR objtype=6.AND.ALLTRIM(comment)=='lhead'        && Координаты вертикальных линий дополнительного заголовка
SCATTER TO dim_lhead                  
DELETE 

LOCATE FOR objtype=8.AND.comment='headsup'                && Координаты наименований дополнитеольного заголовка
SCATTER TO dim_headsup                  

LOCATE FOR objtype=8.AND.ALLTRIM(comment)='mdetail'       && Координаты полей detail
SCATTER TO dim_mdetail                  
DELETE 

LOCATE FOR objtype=8.AND.ALLTRIM(comment)='mdt3'          && Координаты полей detail для 3-го типа
SCATTER TO dim_detail3
DELETE 

LOCATE FOR objtype=8.AND.ALLTRIM(comment)='itogdetail'    && Координаты полей detail-итого
SCATTER TO dim_itogdetail                  
DELETE 

LOCATE FOR objtype=6.AND.comment='ldetail'                && Координаты разделителей detail
SCATTER TO dim_ldetail                  
DELETE 

LOCATE FOR objtype=6.AND.ALLTRIM(comment)='ldetailitog'   && Координаты разделителей detail в итогах
         ...
         CASE EMPTY(fondprn.primlab)
              ...
              IF fondprn.ldouble
                 ...
                 *------- колонка сумма
                 APPEND BLANK
                 GATHER FROM dim_labpers    
                 REPLACE expr WITH "'сумма'",width WITH leftline-supleft-104.67,hpos WITH supleft+104.67*2,fontsize WITH fondprn->fhsize,fontface WITH 'Times New Roman'                                                 
               ENDIF                      
      ENDCASE  
      ncolch=ncolch+1
      SELECT reptarnew
      ...
      *-----detail
      DO CASE
         CASE fondprn.texpr=1
              IF !fondprn.ldouble    && без удвоенных колонок
                  APPEND BLANK 
...
DO WHILE !EOF()     
   ...
   ENDIF     
ENDDO 
DO procForPrintAndPreview WITH 'repOldNew','Тарификационный список'
SELECT peopold
USE
SELECT curPrn
USE
...
DO fltstructure WITH 'mTokl#0','curprn'
stFio=''
st1='1.  Изложить п.8 трудового контракта в следующей редакции:'
st8='8. Работнику устанавливаются следующие условия оплаты труда:'
st81=''
st82=''
st84=''
st86=''
st87=''
st88=''
st83="8.3. надбавка за применение (участие в применении) новых, сложных и уникальных методов оказания медицинской помощи  -  процент оклада (по факту);"

st85='8.5. надбавка врачам-специалистам, должности которых относятся к хирургическому профилю, оказывающим медицинскую помощь в стационарных условиях  40% оклада;'

st89='8.9. доплата за работу во вредных и (или) опасных условиях труда по результатам аттестации рабочих мест.  При установлении доплаты за особый характер труда (п.8.8.) доплата за работу во вредных и (или) опасных условиях труда (п.8.9.)  не выплачивается;' 
st90='9.0. стимулирующие выплаты: премия, надбавка за сложность и напряженность работы, надбавка за высокие достижения в труде  - в процентах от оклада  согласно действующему Положению об оплате труда;'
st91='9.1. единовременная выплата на оздоровление, материальная помощь -  согласно действующему Положению об оплате труда; '
st14='14. Наниматель устанавливает работнику дополнительные меры стимулирования труда:'
st141=''
st142='14.2. дополнительный  отпуск за работу по контракту  _______ календарный   день (календарных дня). '
st2='2. В остальной  части трудовой контракт   оставить без изменений.'
st3='3. Настоящее дополнительное соглашение является неотъемлемой частью контракта и распространяет свое действие на правоотношения, возникшие  с 01 января 2020 года.'
st4='4. Настоящее дополнительное соглашение составлено в двух экземплярах, один из которых хранится у Работника,  другой  - у Нанимателя.'
signn='_______________О.И.Величко'
signr='_______________О.И.Величко'
fiosign=''
SELECT curPrn
GO TOP
DO procForPrintAndPreview WITH 'agreeadd','Дополнительное соглашение'
SELECT rasp
*************************************************
PROCEDURE procstAdd
PARAMETERS par1
...
stFio='          Учреждение здравоохранения "Брестская центральная городская больница" в лице главного врача ВЕЛИЧКО Олега Ивановича (далее - Наниматель), действующего на основании Устава и '+;
       strFio+' (далее - Работник)'
stFio=stFio+' во исполнение Указа Президента Республики Беларусь от 18.01.2019г. № 27 "Об оплате труда работников бюджетных организаций" и приведением,'
stFio=stFio+' на основании части четвертой статьи 19 Трудового кодекса Республики Беларусь, контрактов в соответствие с изменившимся законодательством, заключили настоящее соглашение к контракту о нижеследующем:'
st81='8.1. оклад   в размере '+LTRIM(STR(curprn.mtokl,8,2))+' руб. на день подписания соглашения. В дальнейшем оклад изменяется в соответствии с законодательством;'
st82='8.2. надбавка за стаж работы в бюджетной организации '+IIF(stpr#0,LTRIM(STR(stpr,3)),'________')+' % базовой ставки;' 

st84='8.4. надбавка за специфику работы в сфере здравоохранения   медицинским и фармацевтическим работникам '+IIF(pkat#0,LTRIM(STR(pkat,3)),'________')+' % оклада;'

st86='8.6. доплата за реализацию организационно-распорядительной функции'+IIF(pmain#0.OR.pmain2#0,LTRIM(STR(curprn.pmain+curprn.pmain2,3)),'________')+' % базовой ставки;' 

st87='8.7. надбавка за особенности профессиональной деятельности: фармацевтические работники, инструктор-методист физической реабилитации '+IIF(posob#0,LTRIM(STR(curprn.posob,3)),'________')+' % оклада; '

st88='8.8. доплата за особый характер труда '+IIF(pcharw#0,LTRIM(STR(pcharw,3)),'________')+' % оклада;'
st141='14.1. надбавка за работу на условиях контракта в размере '+IIF(pkont#0,LTRIM(STR(curprn.pkont,3))+'%;','________%;')
...
GO TOP
DO procForPrintAndPreview WITH 'repperstaj','Переходящий стаж'
SELECT people
**************************************************************************************
*   Штатное + персонал
**************************************************************************************
...
DO WHILE !EOF()
   ...
   IF kse_cx#0
      SELECT curprn
      APPEND BLANK
      REPLACE kp WITH curRasp.kp,kd WITH 999,kse WITH kse_cx,np WITH curRasp.np,nd WITH 70,named WITH 'итого',logEnd WITH .T.,ksesh WITH kserasp_cx 
      IF itKat
         SELECT curSuplKat
         SCAN ALL 
              SELECT rasp
...
DO WHILE !EOF()
   ...
   IF kppOld#KodKpp1
      ...
      APPEND BLANK
      REPLACE kp WITH kpOld,kd WITH 999,kse WITH kse_cx,np WITH npOld,nd WITH 80,named WITH 'итого',logEnd WITH .T.,ksesh WITH kserasp_cx 
      IF itKat
         SELECT curSuplKat
         SCAN ALL               
              SELECT curPrn
...
DO WHILE !EOF()
   ...
   IF kppOld#KodKpp
     ...
     * SUM kse TO kserasp_cx FOR SEEK(STR(curprn.kp,3),'rasp',2).AND.rasp.kodKpp=kppOld
      ...
      APPEND BLANK
      REPLACE kp WITH kpOld,kd WITH 999,kse WITH kse_cx,np WITH npOld,nd WITH 90,named WITH 'итого',logEnd WITH .T.,ksesh WITH kserasp_cx 
      IF itKat
         SELECT curSuplKat
         SCAN ALL              
              SELECT curPrn
...
APPEND BLANK 
REPLACE kp WITH 999,kd WITH 999,kse WITH kse_cx,np WITH 999,nd WITH 90,named WITH 'Всего',logEnd WITH .T.,ksesh WITH kserasp_cx
...
SET ORDER TO 1
GO TOP 
DO procForPrintAndPreview WITH 'repspiszpl','Список окладов для зарплатчиков'
SELECT people
**********************************************************************************************
...
WITH excelBook.Sheets(1)
    ...
    .Range(.Cells(rowcx,1),.Cells(rowcx,7)).Select  
     WITH objExcel.Selection
          ...
          .HorizontalAlignment=xlCenter
          .VerticalAlignment=1
          .Value='Список сотрудников для занесения окладов'
          .Font.Name='Times New Roman'   
          .Font.Size=9
     ENDWITH        
     rowcx=rowcx+1                            
                      
     .cells(rowcx,1).Value='Таб.№'
     .cells(rowcx,2).Value='Фамилия Имя Отчество'         
     .cells(rowcx,3).Value='Объем'
     .cells(rowcx,4).Value='Тип'                   
     .cells(rowcx,5).Value='Оклад'
     .cells(rowcx,6).Value='оклад*объем'                    
     .cells(rowcx,7).Value='стаж'              
...
*SET ORDER TO 1
GO TOP 
DO procForPrintAndPreview WITH 'NewOldTable','Список окладов для зарплатчиков'
SELECT people 
**************************************************************************************************************************
...
DO WHILE !EOF()         
   ...
   IF kp#kp_cx
      SELECT curExcel 
      APPEND BLANK
      REPLACE named WITH 'Всего',kp WITH kp_cx,kse WITH ksecx,kse_tot WITH totcx,;
       ...
       IF kpp1_cx#curRaspPrn.kodkpp1.AND.kpp1_cx#0
         SELECT curExcel
         APPEND BLANK      
         REPLACE named WITH 'Всего',kp WITH kp_cx,kse WITH kse_kpp1,kse_tot WITH tot_kpp1,;
     ...
     *--------    
     IF kpp_cx#curRaspPrn.kodkpp.AND.kpp_cx#0      
         SELECT curExcel    
         APPEND BLANK      
         REPLACE named WITH 'Всего',kp WITH kp_cx,kse WITH kse_kpp,kse_tot WITH tot_kpp,;
...
SELECT curExcel
APPEND BLANK
REPLACE named WITH 'Всего по организации',kse WITH ksecx_tot,kse_tot WITH totcx_tot,;
        kse_am WITH amcx_tot,kse_stac WITH staccx_tot,kse_sk WITH skcx_tot,kse_mjr WITH mjrcx_tot,numIt WITH 4
                    
APPEND BLANK
REPLACE named WITH 'Из них:'
FOR i=1 TO maxExcel
...
SELECT curExcel      
GO TOP
IF par1=1   && развернутое
   DO procForPrintAndPreview WITH 'reprasp','Штатное расписание'  
ELSE    && узкое
   DO procForPrintAndPreview WITH 'repraspshort','Штатное расписание'  
ENDIF    
...
objExcel=CREATEOBJECT('EXCEL.APPLICATION')
excelBook=objExcel.workBooks.Add(-4167) 
DIMENSION dim_rows(1)  && массив для строк, где нижние линии отсутствуют
STORE 0 TO dim_rows
SELECT curExcel
DO storezeropercent
max_rows=0 
IF parForm=1 
   WITH excelBook.Sheets(1) 
        ...
        With objExcel.Selection
             ...
             .VerticalAlignment=1
             .WrapText=.T.
             .Value='на '+STR(YEAR(dateTar),4)+' год'            
             .Font.Name='Times New Roman'   
             .Font.Size=12
        ENDWITH  
         rowcx=rowcx+2     
        ...
        With objExcel.Selection
             ...
             .VerticalAlignment=1
             .WrapText=.T.
             .Value='Наименование структурных подразделение и должностей (профессий работников)'
             .Font.Name='Times New Roman'   
             .Font.Size=11
        ENDWITH           
        ...
        With objExcel.Selection
             ...
             .VerticalAlignment=1
             .WrapText=.T.
             .Value='Число должностей (профессий работников) всего'
             .Font.Name='Times New Roman'   
             .Font.Size=10
         ENDWITH       
         ...
         With objExcel.Selection
              ...
              .VerticalAlignment=1
              .WrapText=.T.
              .Value='В том числе по условиям оказание медицинской помощи (услуг)'                    
              .Font.Name='Times New Roman'   
              .Font.Size=11
         ENDWITH                                                 
         ...
         With objExcel.Selection
              ...
              .VerticalAlignment=1
              .WrapText=.T.
              .Value='общий для всех служб персонал'                    
              .Font.Name='Times New Roman'   
              .Font.Size=8
         ENDWITH                        
         ...
         With objExcel.Selection
              ...
              .VerticalAlignment=1
              .WrapText=.T.
              .Value='межрай. отделение переливания крови'
              .Font.Name='Times New Roman'   
              .Font.Size=8
         ENDWITH                                         
...
**********************************************************************************
*          По аттестации рабочих мест (годовая)
**********************************************************************************
PROCEDURE attYear
IF USED('peoplevred')
   SELECT peoplevred
...
ENDFOR
GO TOP
DO procForPrintAndPreview WITH 'repvred1','Надбавка за вредность'
SELECT people
**********************************************************************************
*          По аттестации рабочих мест (годовая)
**********************************************************************************
...
objExcel.Visible=.T.
**********************************************************************************
*                 За стаж годовая (с учётом переходящего cтажа)
**********************************************************************************
PROCEDURE yearStaj
SELECT datJob
SET FILTER TO 
...
SELECT * FROM datJob INTO CURSOR curTarJob READWRITE
DELETE FOR !SEEK(STR(kp,3)+STR(kd,3),'rasp',2)
DELETE FOR date_in>varDtar && удаляем прищедших позже даты т-ции
ALTER TABLE curTarJob ADD COLUMN npp N(3)
ALTER TABLE curTarJob ADD COLUMN nit N(1)
ALTER TABLE curTarJob ADD COLUMN nkat C(200)
SELECT curTarJob
...
SET ORDER TO 1
GO TOP
DO procForPrintAndPreview WITH 'repYearStaj','Годовая ведомость по стажу'

**********************************************************************************
*                Общий свод по окладам, всем видам надбавок, доплат и выплат
**********************************************************************************
PROCEDURE totalsvod
IF USED('curprn')
   SELECT curprn
      ...
      IF rasp.kse#0
         ...
         IF kse_cx>0    
            IF !formbase.vacst                
               SELECT curTarJob
               APPEND BLANK
               REPLACE kp WITH rasp.kp,kd WITH rasp.kd,kat WITH rasp.kat,fio WITH 'Вакантная', kv WITH rasp.kv,kf WITH rasp.kfvac,nameKf WITH rasp.nKfVac,pkf WITH rasp.pkf,pkf WITH rasp.pkf,kse WITH kse_cx,vac WITH .T.
               IF lognzptot
            ...
            ELSE
               ...
               FOR i=1 TO kvovac
                   ...
                   SELECT curTarJob
                   APPEND BLANK
                   REPLACE kp WITH rasp.kp,kd WITH rasp.kd,kat WITH rasp.kat,fio WITH 'Вакантная', kv WITH rasp.kv,kf WITH rasp.kfvac,nameKf WITH rasp.nKfVac,pkf WITH rasp.pkf,kse WITH ksevac,vac WITH .T.,patt WITH rasp.patt
                   IF i=1
                      IF lognzptot
                         REPLACE nzptot WITH rasp.nzptot
                      ELSE   
...
SELECT curPrn
APPEND BLANK
REPLACE name WITH 'Основной оклад',np WITH 0,lDel WITH .T.,lHead WITH .T.
SCAN ALL
     SELECT curTarJob
...
APPEND BLANK
REPLACE name WITH 'Всего',np WITH 499
...
*-------за стаж работы и аттестация рабочих мест
SELECT curPrn
APPEND BLANK
REPLACE name WITH 'Надбавки к заработной плате',np WITH 499,lDel WITH .T.,lHead WITH .T.

APPEND BLANK
REPLACE name WITH 'Надбавка за стаж работы в бюджетных органихациях (Указ №27 п.2)',np WITH 500,nGr WITH 2
APPEND BLANK
REPLACE name WITH 'Доплата по результатам аттестации рабочих мест',np WITH 600,nGr WITH 2
...
SCAN ALL    
     ...
     SELECT curNad
ENDSCAN 
*-------Замена отпусков и курсов
*ON ERROR DO erSup
SELECT curPrn
APPEND BLANK
REPLACE np WITH 601,name WITH 'Выплаты на замену уходящих в отпуск',nGr WITH 2
*APPEND BLANK
*REPLACE np WITH 602,name WITH 'Выплаты на замену уходящих на курсы',nGr WITH 2
IF lognzptot
   SELECT curtarjob
...
SELECT curPrn
APPEND BLANK
REPLACE np WITH 603,name WITH 'Выплаты за работу в ночное время',nGr WITH 2
APPEND BLANK
REPLACE np WITH 604,name WITH 'Выплаты за работу в праздничные дни',nGr WITH 2
SELECT curTarRasp
SCAN ALL
     FOR i=MONTH(dateTar) TO 12
...
APPEND BLANK
REPLACE name WITH 'Всего',np WITH 700
REPLACE mtot WITH mtotcx,m1 WITH m1cx,m2 WITH m2cx,m3 WITH m3cx,m4 WITH m4cx,m5 WITH m5cx,m6 WITH m6cx,m7 WITH m7cx,m8 WITH m8cx,m9 WITH m9cx,m10 WITH m10cx,m11 WITH m11cx,m12 WITH m12cx
...
APPEND BLANK
REPLACE name WITH 'Итого',np WITH 701
REPLACE mtot WITH mtotcx,m1 WITH m1cx,m2 WITH m2cx,m3 WITH m3cx,m4 WITH m4cx,m5 WITH m5cx,m6 WITH m6cx,m7 WITH m7cx,m8 WITH m8cx,m9 WITH m9cx,m10 WITH m10cx,m11 WITH m11cx,m12 WITH m12cx
SELECT curPrn
DELETE FOR mtot=0.AND.!lDel
SET ORDER TO 1
GO TOP
DO procForPrintAndPreview WITH 'repSvod','Сводная ведомость по надбавкам, доплатам и прочим выплатам'
**********************************************************************************
*          Своднаа в excel
**********************************************************************************
...
WITH excelBook.Sheets(1)
     ...
     WITH objExcel.Selection
          ...
          .VerticalAlignment=1
          .WrapText=.T.
          .Value='Сводня ведомость по окладам, надбавкам, доплатам и прочим выплатам на '+STR(YEAR(dateTar),4)+' г.'
          .Font.Name='Times New Roman'   
          .Font.Size=9
     ENDWITH        
     rowcx=rowcx+1  
     ...
     With objExcel.Selection
          ...
          .VerticalAlignment=1
          .WrapText=.T.
          .Value='Наименование подразделения, надбавок, доплат и прочих выплат'
          .Font.Name='Times New Roman'   
          .Font.Size=9
     ENDWITH          
...
SCAN ALL
     IF ksepps#0.AND.nspps#0
        SELECT curprn
        APPEND BLANK
        REPLACE kp WITH rasp.kp,kd WITH rasp.kd,kse WITH rasp.ksepps,fio WITH 'Вакантная',vac WITH .T.,npps WITH rasp.npps,nopps WITH rasp.nopps,nspps WITH rasp.nspps
     ENDIF 
     SELECT rasp
ENDSCAN
...
SCAN ALL
     ...
     IF sprncx#0
        APPEND BLANK
        REPLACE fio WITH 'Всего',kp WITH sprpodr.kod,np WITH sprpodr.np,nd WITH 500,nspps WITH sprncx,kse WITH ksecx,nIt WITH 1     
     ENDIF
     IF itkat
        FOR i=1 TO maxKat
            SELECT curPrn
...
IF sprncx#0
   APPEND BLANK
   REPLACE fio WITH IIF(formbase.lItr,'ИТОГО','Итого'),kp WITH 999,np WITH 999,nd WITH 500,nspps WITH sprncx,kse WITH ksecx,nIt WITH 9  
   IF formbase.litr
      FOR i=1 TO max_tr
   ...
   ENDFOR     
ENDIF	
DO procForPrintAndPreview WITH 'reppps','Тарификационный список'
SELECT curPrn
USE
SELECT people
**********************************************************************************************
...
WITH excelBook.Sheets(1)
     ...
     WITH objExcel.Selection
          ...
          .VerticalAlignment=1
          .WrapText=.T.
          .Value='Расчёт расходов на выплату доплат по ППС'
          .Font.Name='Times New Roman'   
          .Font.Size=9
     ENDWITH  
...
WITH excelBook.Sheets(1)
     ...
     .cells(rowcx+1,3).Value='всего'
     .cells(rowcx+1,4).Value='без отработки'
     .cells(rowcx+1,5).Value='с отработкой'
...
SET ORDER TO 1
GO TOP
DO procForPrintAndPreview WITH 'reptbltot','сравнительные таблицы'
SELECT curprn
USE
SELECT curcomprn
USE
