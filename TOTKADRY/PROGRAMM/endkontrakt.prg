DIMENSION dim_ord(3)
dim_ord(1)=1 && дата окончания
dim_ord(2)=0 &&  алфавитный режим
dim_ord(3)=0 && штатный режим
STORE CTOD('  .  .    ') TO dBeg_cx,dEnd_cx

DIMENSION dim_knt(3)
dim_knt(1)=1 && контракт
dim_knt(2)=0 &&  договор
dim_knt(3)=0 && контракт+договор


DO procDimFlt
fSupl=CREATEOBJECT('FORMSUPL')
WITH fSupl
     .Caption='Даты окончания контрактов'
     .Icon='kone.ico'
     .procexit='Do exitend' 
     DO procObjFlt     
     DO addshape WITH 'fSupl',2,.Shape1.Left,.Shape1.Top+.Shape1.Height+10,150,.Shape1.Width,8     
        
     DO adLabMy WITH 'fSupl',1,'период окончания',.Shape2.Top+10,.Shape2.Left+5,.Shape2.Width-10,2,.F.,1 
     ...
     DO addOptionButton WITH 'fSupl',11,'контракт',.txtBox1.Top+.txtBox1.Height+10,.Shape2.Left+20,'dim_knt(1)',0,"DO procValOption WITH 'fSupl','dim_knt',1",.T. 
     DO addOptionButton WITH 'fSupl',12,'договор',.Option11.Top,.Option11.Left+.Option11.Width+20,'dim_knt(2)',0,"DO procValOption WITH 'fSupl','dim_knt',2",.T. 
     DO addOptionButton WITH 'fSupl',13,'контракт+договор',.Option11.Top,.Option12.Left+.Option12.Width+20,'dim_knt(3)',0,"DO procValOption WITH 'fSupl','dim_knt',3",.T. 
     ...
     DO addOptionButton WITH 'fSupl',1,'дата окончания',.Option11.Top+.Option11.Height+10,.Shape2.Left+20,'dim_ord(1)',0,"DO procValOption WITH 'fSupl','dim_ord',1",.T. 
     DO addOptionButton WITH 'fSupl',2,'режим алфавитный',.Option1.Top,.Option1.Left+.Option1.Width+20,'dim_ord(2)',0,"DO procValOption WITH 'fSupl','dim_ord',2",.T. 
     DO addOptionButton WITH 'fSupl',3,'режим штатный',.Option1.Top,.Option1.Left+.Option1.Width+20,'dim_ord(3)',0,"DO procValOption WITH 'fSupl','dim_ord',3",.T. 
...
SCAN ALL
     ...
     SEEK curprn.num
     SELECT curprn
     REPLACE kp WITH kontJob.kp,kd WITH kontJob.kd,cnamep WITH IIF(SEEK(kp,'sprpodr',1),sprpodr.namework,''),cnamed WITH IIF(SEEK(kd,'sprdolj',1),sprdolj.name,''),cprim WITH IIF(dekotp,'д/о',''),kat WITH kontJob.kat
ENDSCAN
...
DO CASE 
   CASE par1=1
        DO procForPrintAndPreview WITH 'rependkont','окончание контрактов',.T.,'endKontToExcel'
   CASE par1=2
        DO procForPrintAndPreview WITH 'rependkont','окончание контрактов',.F.,'endKontToExcel'
ENDCASE 
********************************************************************************************************
...
WITH excelBook.Sheets(1)
     ...
     .cells(2,5).Value='дата окончания'
