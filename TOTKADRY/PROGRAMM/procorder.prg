...
STORE 0 TO dim_type
dim_type(1)=1
newKodOrder=0   && код приказа  
typeOrdNew=1  && тип приказа 
numOrdNew=0   && номер приказа
dateOrdNew=CTOD('  .  .    ')
strOrdNew='а' && обозначение приказа
logNew=.F.
logNewRec=.F.
logRead=.F.
newSlink=''
str_ini=''
objFocus=''  && объект для перевода фокуса после выбора из listbox
parPadejDol=1
parPadejFio=1
newLogApp=.T.
logDatJob=.F.
...
WITH fOrdStart
     ...
     DO addOptionButton WITH 'fOrdStart',1,'Административный',.Shape1.Top+20,.Shape1.Left+20,'dim_type(1)',0,"DO procValTypeOrd WITH 1",.T.     
     DO addOptionButton WITH 'fOrdStart',2,'Кадровый',.Option1.Top,.Option1.Left+.Option1.Width+10,'dim_type(2)',0,"DO procValTypeOrd WITH 2",.T.     
     DO addOptionButton WITH 'fOrdStart',3,'Отпуск',.Option1.Top,.Option2.Left+.Option2.Width+10,'dim_type(3)',0,"DO procValTypeOrd WITH 3",.T.
     DO addOptionButton WITH 'fOrdStart',4,'Командировки',.Option1.Top+.Option1.Height+10,.Option3.Left+.Option3.Width+10,'dim_type(4)',0,"DO procValTypeOrd WITH 4",.T.
     DO adtBoxAsCont WITH 'fOrdStart','contNum',10,.Option4.Top+.Option4.Height+10,RetTxtWidth('WНомер приказаW'),dHeight,'номер приказа',2,1  
     DO adtBoxAsCont WITH 'fOrdStart','contDate',.contNum.Left+.contNum.Width-1,.contNum.Top,RetTxtWidth('WДата приказаW'),dHeight,'дата приказа',2,1  
    ...
    * .tBox2.Left=.tBox1.Left+.tBox1.Width-1
     ...
     WITH .grdOrd
          ...
          .Column2.Header1.Caption='тип'
          .Column3.Header1.Caption='дата'
          .Column4.Header1.Caption='наименование'
          .Column5.Header1.Caption='п'
     ...
     DO addButtonOne WITH 'fOrdStart','butStart',.shape1.Left+(.Shape1.Width-(RetTxtWidth('wприступитьw')*3)-20)/2,.grdOrd.Top+.grdOrd.Height+20,'приступить','','DO formforOrder',dHeight+5,RetTxtWidth('wприступитьw'),'новая запись'  
     DO addButtonOne WITH 'fOrdStart','butRead',.butStart.Left+.butStart.Width+10,.butStart.Top,'номер-дата','','DO changeNumOrd',.butStart.Height,.butStart.Width,'редактировать номер-дата'  
     DO addButtonOne WITH 'fOrdStart','butRet',.butRead.Left+.butRead.Width+10,.butStart.Top,'возврат','','fOrdStart.Release',.butStart.Height,.butStart.Width,'возврат'      


     DO addButtonOne WITH 'fOrdStart','butSave',.shape1.Left+(.Shape1.Width-(RetTxtWidth('wзаписатьw')*2)-10)/2,.butStart.Top,'записать','','DO saveNumOrder WITH .T.',.butStart.Height,RetTxtWidth('wзаписатьw'),'записать'  
     DO addButtonOne WITH 'fOrdStart','butReatRead',.butSave.Left+.butSave.Width+10,.butSave.Top,'отказ','','DO saveNumOrder WITH .F.',.butSave.Height,.butSave.Width,'отказ'       
     .butSave.Visible=.F.
...
IF par1
   ...
   SELECT datotp
   REPLACE dord WITH datOrder.dateOrd,nord WITH LTRIM(STR(datorder.numord)+'-'+ALLTRIM(datorder.strord)),osnov WITH 'пр.№ '+LTRIM(STR(datOrder.numOrd))+'-'+ALLTRIM(datOrder.strOrd)+' от '+DTOC(datOrder.dateOrd) FOR kord=datorder.kod
ELSE 
ENDIF 
WITH fOrdStart
...
DO CASE
   ...
   CASE typeOrdNew=2 
        SELECT curSupOrder
        APPEND BLANK
        REPLACE kod WITH 1,name WITH 'принять'
        APPEND BLANK
        REPLACE kod WITH 2,name WITH 'перевести'
        APPEND BLANK
        REPLACE kod WITH 3,name WITH 'контракт'        
        APPEND BLANK
        REPLACE kod WITH 4,name WITH 'уволить'
        APPEND BLANK
        REPLACE kod WITH 5,name WITH 'установить'
...
dim_agree(1)='ознакомлен, согласен:'
dim_agree(2)='ознакомлена, согласна:'

SELECT peoporder
SEEK newKodOrder
DIMENSION dim_nkse(2)
dim_nkse(1)='штатные единицы'
dim_nkse(2)='должности'
...
WITH frmOrd
     ...
     DO adTboxAsCont WITH 'frmOrd','ordNum',0,.ordTop.Top,RetTxtWidth('WПриказ № '),dHeight,'Приказ № ',2,1 
     DO adTboxNew WITH 'frmOrd','tBox1',.ordTop.Top,.ordTop.Left,RetTxtWidth('9999999'),dHeight,'numOrdNew','Z',.F.,0 
     DO adTboxAsCont WITH 'frmOrd','ordType',0,.ordTop.Top,RetTxtWidth('wКw'),dHeight,strOrdNew,2,1      
     DO adTboxAsCont WITH 'frmOrd','ordDate',0,.ordTop.Top,RetTxtWidth('Wдата'),dHeight,'Дата',2,1  
     ...
     DO adCheckBox WITH 'frmOrd','checkClose','завершить приказ',.ordTop.Top,.tBox2.Left+.tBox2.Width+5,150,dHeight,'newLotm',1,.F.,'DO validcheckClose' 
     ...
     ENDFOR
     IF typeOrdNew=2
        DO adTboxAsCont WITH 'frmOrd','ordSupPrik',.ordNum.Left,.grdPers.Top,RetTxtWidth('WФамилия Имя ОтчествоW'),dHeight,'Тип приказа',1,1 
        ...
        DO adTboxAsCont WITH 'frmOrd','ordPrik',.ordNum.Left,.ordSupPrik.Top+.ordSupPrik.Height-1,.ordSupPrik.Width,dHeight,'Приказываю',1,1 
     ...
     ELSE  
        DO adTboxAsCont WITH 'frmOrd','ordPrik',.ordNum.Left,.grdPers.Top,RetTxtWidth('WФамилия Имя ОтчествоW'),dHeight,'Тип приказа',1,1 
        DO addcombomy WITH 'frmOrd',1,.ordPrik.Left+.ordPrik.Width-1,.ordPrik.Top,dHeight,.Width-.grdPers.Width-.ordPrik.Width+2,IIF(logNewRec,.T.,.F.),'strPrik','ALLTRIM(curSprOrder.nameOrd)',6,'','DO procPrikOrd',.F.,.T.       
        .comboBox1.Enabled=.F.
        .comboBox1.DisplayCount=20
        .comboBox1.DisabledForeColor=RGB(1,0,0)       
     ...
     DO addButtonOne WITH 'frmOrd','butNew',.ordPrik.Left,.Height-59,'новая','','DO newRecInOrder WITH .T.',39,RetTxtWidth('сохранитьw'),'новая запись'  
     DO addButtonOne WITH 'frmOrd','butRead',.ordPrik.Left,.butNew.Top,'редакция','','DO newRecInOrder WITH .F.',39,.butNew.Width,'редакция'  
     DO addButtonOne WITH 'frmOrd','butDel',.ordPrik.Left,.butNew.Top,'удаление','','DO delFromOrder',39,.butNew.Width,'удаление'      
     DO addButtonOne WITH 'frmOrd','butPrn',.ordPrik.Left,.butNew.Top,'печать','','DO formPrnOrder',39,.butNew.Width,'печать'
     DO addButtonOne WITH 'frmOrd','butSearch',.ordPrik.Left,.butNew.Top,'поиск','','DO procSearchOrder',39,.butNew.Width,'поиск' 
     DO addButtonOne WITH 'frmOrd','butRet',.ordPrik.Left,.butNew.Top,'возврат','','DO procExitOrder',39,.butNew.Width,'возврат' 
     ...
     DO addButtonOne WITH 'frmOrd','butTxt',.ordPrik.Left,.butNew.Top,'текст','','DO formTextOrder',39,.butNew.Width,'сформировать текст' 
     DO addButtonOne WITH 'frmOrd','butSaveRec',.ordPrik.Left,.butNew.Top,'записать','','DO saveRecOrder WITH .T.',39,.butNew.Width,'записать' 
     DO addButtonOne WITH 'frmOrd','butRetRead',.ordPrik.Left,.butNew.Top,'возврат','','DO saveRecOrder WITH .F.',39,.butNew.Width,'возврат' 
     .butTxt.Visible=.F.
     ...
     DO addButtonOne WITH 'frmOrd','butRetNew',.ordPrik.Left,.butNew.Top,'возврат','','DO procButRetNew',39,.butNew.Width,'возврат' 
     .butRetNew.Left=.ordPrik.Left+(.ordPrik.Width+.comboBox1.Width-.butRetNew.Width)/2
     .butRetNew.Visible=.F.     
     DO addButtonOne WITH 'frmOrd','butDelRec',.ordPrik.Left,.butNew.Top,'удалить','','DO delRecOrder WITH .T.',39,.butNew.Width,'удалить запись' 
     DO addButtonOne WITH 'frmOrd','butDelRet',.ordPrik.Left,.butNew.Top,'возврат','','DO delRecOrder WITH .F.',39,.butNew.Width,'возврат' 
     .butDelRec.Visible=.F.
     .butDelRet.Visible=.F.
     ...
     DO addButtonOne WITH 'frmOrd','butSearchRec',.tBoxSearch.Left+.tBoxSearch.Width+5,.butNew.Top,'поиск','','DO searchRecOrder',39,.butNew.Width,'поиск запись' 
     DO addButtonOne WITH 'frmOrd','butSearchNext',.butSearchRec.Left,.butNew.Top,'далее','','DO searchNextRecOrder',39,.butNew.Width,'искать далее' 
     DO addButtonOne WITH 'frmOrd','butSearchRet',.butSearchRec.Left+.butSearchRec.Width+5,.butNew.Top,'возврат','','DO searchRetOrder',39,.butNew.Width,'возврат' 
...
* parPadejDol=1 Родительный - кого - people.fior,sprdolj.namer
* parPadejDol=2 Дательный - кому - people.fiod,sprdolj.named
* parPadejDol=3 Винительный или творительный - кем - people.fiov,sprdolj.namet
...
IF logNewRec.AND.logPer
   ...
   DELETE FOR kodotp>3   
   GO TOP
   LOCATE FOR !EMPTY(perEnd) && подстраховка для сведений со старой программы и поиск последнего отпуска
   IF kodotp=1
...
WITH frmOrd 
     DO addObjectOrder    
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordFio.Top+dHeight-1,.ordPrik.Width,dHeight,'Принять с',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxBeg',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateBeg',.F.,IIF(par1,.T.,.F.),0      
     DO adTboxAsCont WITH 'frmOrd','ordEnd',.ordPrik.Left,.ordBeg.Top,RetTxtWidth('поw'),dHeight,'по',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPodr',.ordPrik.Left,.ordBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'Подразделение',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDol',.ordPrik.Left,.ordPodr.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordKse',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Объём',1,1  
     ...
     DO adTBoxAsCont WITH 'frmOrd','ordTr',.spinKse.Left+.spinKse.Width-1,.ordKse.Top,RetTxtWidth('wтипw'),dHeight,'тип',2,1                                             
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPkont',.ordPrik.Left,.ordKse.Top+dHeight-1,.ordPrik.Width,dHeight,'% за контракт',1,1  
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDay',.ordPrik.Left,.ordPkont.Top+dHeight-1,.ordPrik.Width,dHeight,'поощр.отпуск',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxDay',.ordDay.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDDop','Z',IIF(par1,.T.,.F.),0,'99'       
     .tBoxDay.Alignment=0
     
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,.ordDay.Top+dHeight-1,.ordPrik.Width,dHeight,'основание',1,1  
...
ENDWITH
********************************************************************************************************************************
*                         Формирование текстовой части приказа приема на условиях контракта
********************************************************************************************************************************
...
SELECT txtOrder
REPLACE txtprn WITH 'Принять '+ALLTRIM(cOrdFio)+' с '+strDateBeg+' на работу на условиях контракта по '+strDateEnd+' '+;
        LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' с оплатой согласно штатному расписанию , установив дополнительные меры стимулирования:'+CHR(13)+;
        '     Надбавка за работу на условиях контракта - '+LTRIM(STR(newPkont))+' %;'+CHR(13)+;
        '     Предоставление дополнительного поощрительного трудового отпуска - '+LTRIM(STR(newDDop))+' календарный день.'+CHR(13)+CHR(13)+;
        '     С приказом '+dim_agree(kodsex)+CHR(13)+;
        '     С правилами внутреннего трудового распорядка и '+CHR(13)+;
        '     коллективным договором '+dim_agree(kodsex)+CHR(13)+CHR(13)+;
        'Основание: '+ALLTRIM(newOsnov)       
...
DO saveKadrOrder
********************************************************************************************************************************
*       Прием на условиях контракта (множественный)
********************************************************************************************************************************
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPodr2',.ordPrik.Left,.ordTr.Top+dHeight-1,.ordPrik.Width,dHeight,'Подразделение',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDol2',.ordPrik.Left,.ordPodr2.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordKse2',.ordPrik.Left,.ordDol2.Top+dHeight-1,.ordPrik.Width,dHeight,'Объём',1,1  
     ...
     *****3
     DO adTboxAsCont WITH 'frmOrd','ordPodr3',.ordPrik.Left,.ordKse2.Top+dHeight-1,.ordPrik.Width,dHeight,'Подразделение',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDol3',.ordPrik.Left,.ordPodr3.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordKse3',.ordPrik.Left,.ordDol3.Top+dHeight-1,.ordPrik.Width,dHeight,'Объём',1,1  
      ...
      *****43
     DO adTboxAsCont WITH 'frmOrd','ordPodr4',.ordPrik.Left,.ordKse3.Top+dHeight-1,.ordPrik.Width,dHeight,'Подразделение',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDol4',.ordPrik.Left,.ordPodr4.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordKse4',.ordPrik.Left,.ordDol4.Top+dHeight-1,.ordPrik.Width,dHeight,'Объём',1,1  
     ...
     **Остальное
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPkont',.ordPrik.Left,.ordKse4.Top+dHeight-1,.ordPrik.Width,dHeight,'% за контракт',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxKont',.ordPkont.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newPkont','Z',IIF(par1,.T.,.F.),0,'999'     
     .tBoxKont.Alignment=0
...
KEYBOARD '{TAB}'
********************************************************************************************************************************
*                         Формирование текстовой части приказа приема на условиях контракта (множественный)
********************************************************************************************************************************
...
strmany=' (из них '
IF newKse#0
   strmany=strmany+STR(newKse,4,2)+' должности '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' '
ENDIF
...
IF newKse2#0 
   ...
   ELSE
      cOrdDol2=newNDolj2
   ENDIF    
   strmany=strmany+STR(newKse2,4,2)+' должности '+LOWER(cOrdDol2)+' '+LOWER(cOrdPodr2)+' '
ENDIF
...
IF newKse3#0
   ...
   ELSE
      cOrdDol3=newNDolj3
   ENDIF       
   strmany=strmany+STR(newKse3,4,2)+' должности '+LOWER(cOrdDol3)+' '+LOWER(cOrdPodr3)+' '
ENDIF
...
IF newKse4#0
   ...
   ELSE
      cOrdDol4=newNDolj4
   ENDIF 
   strmany=strmany+STR(newKse4,4,2)+' должности '+LOWER(cOrdDol4)+' '+LOWER(cOrdPodr4)+' '
ENDIF
strmany=strmany+')'
SELECT txtOrder
REPLACE txtprn WITH 'Принять '+ALLTRIM(cordFio)+' с '+strDateBeg+' на работу на условиях контракта по '+strDateEnd+' '+;
        LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+strmany+' с оплатой согласно штатному расписанию , установив дополнительные меры стимулирования:'+CHR(13)+;
        '     Надбавка за работу на условиях контракта - '+LTRIM(STR(newPkont))+' %;'+CHR(13)+;
        '     Предоставление дополнительного поощрительного трудового отпуска - '+LTRIM(STR(newDDop))+' календарный день.'+CHR(13)+CHR(13)+;
        '     С приказом '+dim_agree(kodsex)+CHR(13)+;
        '     С правилами внутреннего трудового распорядка и '+CHR(13)+;
...
DO saveKadrOrder
********************************************************************************************************************************
*       Прием на условиях внешнего совместительства
********************************************************************************************************************************
PROCEDURE priemvn
PARAMETERS par1
IF !USED('sprtot')
...
ENDIF
IF !USED('curSrokDog')
   SELECT kod,name FROM sprtot WHERE sprtot.kspr=25 INTO CURSOR curSrokDog READWRITE && курсор для видов трудовых дооговоров
   SELECT curSrokDog
   INDEX ON kod TAG T1
ENDIF  
parPadejDol=3
...
WITH frmOrd 
     DO addObjectOrder 
     DO adTboxAsCont WITH 'frmOrd','ordPodr2',.ordPrik.Left,.ordFio.Top+dHeight-1,.ordPrik.Width,dHeight,'трудовой договор',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordPodr2.Top+dHeight-1,.ordPrik.Width,dHeight,'Принять с',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxBeg',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateBeg',.F.,IIF(par1,.T.,.F.),0  
     ...
     DO adTboxAsCont WITH 'frmOrd','ordFioNew',.ordPrik.Left,.ordTr.Top+dHeight-1,.ordPrik.Width,dHeight,'ФИО (кого замещают)',1,1      
     ...
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,.ordFioNew.Top+dHeight-1,.ordPrik.Width,dHeight,'основание',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0  
...
ENDCASE
********************************************************************************************************************************
*                         Формирование текстовой части приказа приема  внешнего совместителя
********************************************************************************************************************************
...
DO CASE
   CASE newVdog=1 
        REPLACE txtprn WITH 'Принять '+ALLTRIM(cOrdFio)+' с '+strDateBeg+' по '+strDateEnd+' на работу в порядке внешнего совместительства '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' на '+STR(newKse,4,2)+;
                ' штатные единицы с оплатой согласно штатному расписанию'+CHR(13)+CHR(13)+;
                '     С приказом '+dim_agree(kodsex)+CHR(13)+;               
                '     С правилами внутреннего трудового распорядка и '+CHR(13)+;
                '     коллективным договором '+dim_agree(kodsex)+CHR(13)+CHR(13)+;
                'Основание: '+ALLTRIM(newOsnov)       
   ...
   CASE newVdog=2 
        REPLACE txtprn WITH 'Принять '+ALLTRIM(cOrdFio)+' с '+strDateBeg+' на работу в порядке внешнего совместительства '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' на '+STR(newKse,4,2)+;
                ' штатные единицы на период отсутстви основного работника '+ALLTRIM(newFioZam) +CHR(13)+CHR(13)+;
                '     С приказом '+dim_agree(kodsex)+CHR(13)+;               
                '     С правилами внутреннего трудового распорядка и '+CHR(13)+;
                '     коллективным договором '+dim_agree(kodsex)+CHR(13)+CHR(13)+;
                'Основание: '+ALLTRIM(newOsnov)  
...
DO saveKadrOrder
********************************************************************************************************************************
*       Прием на условиях трудового договора
********************************************************************************************************************************
PROCEDURE priemDog 
...
**par2=1  срочный
**par2=2  на время отсутствия основного работника
**par2=3  на неопределенный срок
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDay',.ordPrik.Left,.ordTr.Top+dHeight-1,.ordPrik.Width,dHeight,'поощр.отпуск',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxDay',.ordDay.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDDop','Z',IIF(par1,.T.,.F.),0,'99'       
     .tBoxDay.Alignment=0
               
     IF INLIST(nuvol,2,4)
        DO adTboxAsCont WITH 'frmOrd','ordFioNew',.ordPrik.Left,.ordDay.Top+dHeight-1,.ordPrik.Width,dHeight,'ФИО (кого замещают)',1,1      
     ...
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,IIF(INLIST(nuvol,2,4),.ordFioNew.Top,.ordDay.Top)+dHeight-1,.ordPrik.Width,dHeight,'основание',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0 
...
DO CASE
   CASE nuvol=1
        REPLACE txtprn WITH 'Принять '+cOrdFio+' с '+strDateBeg+' по '+strDateEnd+' '+;
        LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' с оплатой согласно штатному расписанию.'+CHR(13)+;
        '     С приказом '+dim_agree(kodsex)+CHR(13)+;
        '     С правилами внутреннего трудового распорядка и '+CHR(13)+;
        '     коллективным договором '+dim_agree(kodsex)+CHR(13)+CHR(13)+;
        'Основание: '+ALLTRIM(newOsnov)       
   CASE nuvol=2
        REPLACE txtprn WITH 'Принять '+cOrdFio+' на работу с '+strDateBeg+' '+;
        LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+;
        ', с оплатой согласно штатному расписанию, на период отсутствия основного работника '+ALLTRIM(newFioZam)+', находящейся в социальном отпуске "по уходу за ребенком до достижения им возраста 3-х лет".'+CHR(13)+;
        '     С приказом '+dim_agree(kodsex)+CHR(13)+;
        '     С правилами внутреннего трудового распорядка и '+CHR(13)+;
        '     коллективным договором '+dim_agree(kodsex)+CHR(13)+CHR(13)+;
        'Основание: '+ALLTRIM(newOsnov)  
   CASE nuvol=3
        ...
        'Основание: '+ALLTRIM(newOsnov)          
   CASE nuvol=4
        REPLACE txtprn WITH 'Принять '+cOrdFio+' на работу в порядке внешнего совместительства с '+strDateBeg+' '+;
        LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+;
        ', с оплатой согласно штатному расписанию, на период отсутствия основного работника '+ALLTRIM(newFioZam)+', находящейся в социальном отпуске "по уходу за ребенком до достижения им возраста 3-х лет."'+CHR(13)+;
        '     С приказом '+dim_agree(kodsex)+CHR(13)+;
        '     С правилами внутреннего трудового распорядка и '+CHR(13)+;
...
ENDIF
IF !USED('curPerevod')
   SELECT kod,name FROM sprtot WHERE sprtot.kspr=26 INTO CURSOR curPerevod READWRITE && курсор для видов приема на условиях перевода
   SELECT curPerevod
   INDEX ON kod TAG T1
ENDIF  
parPadejDol=3
...
WITH frmOrd 
     DO addObjectOrder 
     DO adTboxAsCont WITH 'frmOrd','ordPodr2',.ordPrik.Left,.ordFio.Top+dHeight-1,.ordPrik.Width,dHeight,'перевод на условиях',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPkont',.ordPrik.Left,.ordTr.Top+dHeight-1,.ordPrik.Width,dHeight,'% за контракт',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxKont',.ordPkont.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newPkont','Z',IIF(par1,.T.,.F.),0,'999'     
     .tBoxKont.Alignment=0
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDolNew',.ordPrik.Left,.ordFioNew.Top+dHeight-1,.ordPrik.Width,dHeight,'перевод из',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,.ordDolNew.Top+dHeight-1,.ordPrik.Width,dHeight,'основание',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0  
...
DO CASE
   CASE newVdog=1 
        REPLACE txtprn WITH 'Принять '+cOrdFio+' с '+strDateBeg+' на работу на условиях контракта по '+strDateEnd+' '+;
        LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' с оплатой согласно штатному расписанию, в порядке перевода из '+ALLTRIM(newPlace)+', установив дополнительные меры стимулирования:'+CHR(13)+;
        '     Надбавка за работу на условиях контракта - '+LTRIM(STR(newPkont))+' %;'+CHR(13)+;
        '     Предоставление дополнительного поощрительного трудового отпуска - '+LTRIM(STR(newDDop))+' календарный день.'+CHR(13)+CHR(13)+;
        '     С приказом '+dim_agree(kodsex)+CHR(13)+;
        '     С правилами внутреннего трудового распорядка и '+CHR(13)+;
        ...
        'Основание: '+ALLTRIM(newOsnov)
   CASE newVdog=2 
        REPLACE txtprn WITH 'Принять '+cOrdFio+' с '+strDateBeg+' на работу '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' на '+STR(newKse,4,2)+;
                ' штатные единицы порядке перевода из '+ALLTRIM(newPlace)+' на период отсутствия основного работника '+ALLTRIM(newFioZam) +CHR(13)+CHR(13)+'Основание:'+newOsnov+CHR(13)+' С приказом '+dim_agree(kodsex)
   CASE newVdog=3 
        REPLACE txtprn WITH 'Принять '+cOrdFio+' с '+strDateBeg+' на работу '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' на '+STR(newKse,4,2)+;
                ' штатные единицы с оплатой согласно штатному расписанию порядке перевода из '+ALLTRIM(newPlace)+CHR(13)+'Основание:'+newOsnov+CHR(13)+CHR(13)+' С приказом '+dim_agree(kodsex)                                
ENDCASE        
********************************************************************************************************************************
...
IF !USED('curSrok')
   ...
   INDEX ON kod TAG T1
ENDIF  
*par2=1  заключить контракт
*par2=2  заключить контракт на ранее заключенных условиях
*par2=2  продлить контракт на ранее заключенных условиях
parSave=par2
parPadejDol=3
parPadejFio=3
DO newNidOrder
...
WITH frmOrd 
     DO addObjectOrder
     DO adTboxAsCont WITH 'frmOrd','ordPodr',.ordPrik.Left,.ordFio.Top+dHeight-1,.ordPrik.Width,dHeight,'Подразделение',1,1 
     DO adTboxNew WITH 'frmOrd','tBoxPodr',.ordPodr.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNpodr',.F.,.F.,0  
     DO adTboxAsCont WITH 'frmOrd','ordDol',.ordPrik.Left,.ordPodr.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность',1,1 
     DO adTboxNew WITH 'frmOrd','tBoxDolj',.ordDol.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNDolj',.F.,.F.,0 
     
     
     DO adTboxAsCont WITH 'frmOrd','ordNKont',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'контракт №',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxNKont',.ordNKont.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNKont',.F.,IIF(par1,.T.,.F.),0      
     DO adTboxAsCont WITH 'frmOrd','ordDKont',.ordPrik.Left,.ordNKont.Top,RetTxtWidth('wдата контрактаw'),dHeight,'дата контракта',1,1   
     ...
     DO adTBoxAsCont WITH 'frmOrd','ordTr',.ordPrik.Left,.ordNKont.Top+dHeight-1,.ordPrik.Width,dHeight,'на срок',1,1                                             
     ...
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordTr.Top+dHeight-1,.ordPrik.Width,dHeight,'заключить с',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxBeg',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateBeg',.F.,IIF(par1,.T.,.F.),0      
     DO adTboxAsCont WITH 'frmOrd','ordEnd',.ordPrik.Left,.ordBeg.Top,RetTxtWidth('поw'),dHeight,'по',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxEnd',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateEnd',.F.,IIF(par1,.T.,.F.),0  
     .tBoxBeg.Width=(.comboBox1.Width-.ordEnd.Width)/2
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPkont',.ordPrik.Left,.ordBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'% за контракт',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxKont',.ordPkont.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newPkont','Z',IIF(par1,.T.,.F.),0,'999'     
     .tBoxKont.Alignment=0
...
DO CASE
   CASE parSave=1
        REPLACE txtprn WITH 'Заключить с '+cOrdFio+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', новый трудовой контракт сроком на '+ALLTRIM(strSrok)+' с '+;
        strDateBeg+' по '+strDateEnd+', установив дополнительные меры стимулирования:'+CHR(13)+;
                '     Надбавка за работу на условиях контракта - '+LTRIM(STR(newPkont))+' %;'+CHR(13)+;
                '     Предоставление дополнительного поощрительного трудового отпуска - '+LTRIM(STR(newDDop))+' календарный день.'+CHR(13)+CHR(13)+;
                'Основание: '+ALLTRIM(newOsnov)          
    CASE parSave=2   
         REPLACE txtprn WITH 'Заключить с '+cOrdFio+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', новый трудовой контракт сроком на '+ALLTRIM(strSrok)+' с '+;
                  strDateBeg+' по '+strDateEnd+', на ранее заключенных условиях'+CHR(13)+CHR(13)+;                
                 'Основание: '+ALLTRIM(newOsnov)          
    CASE parSave=3 
         REPLACE txtprn WITH 'Продлить с '+cOrdFio+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', новый трудовой контракт сроком на '+ALLTRIM(strSrok)+' с '+;
                 strDateBeg+' по '+strDateEnd+', на ранее заключенных условиях'+CHR(13)+CHR(13)+;                
                'Основание: '+ALLTRIM(newOsnov)   
 ENDCASE     
********************************************************************************************************************************
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDol',.ordPrik.Left,.ordPodr.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность',1,1 
     DO adTboxNew WITH 'frmOrd','tBoxDolj',.ordDol.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNDolj',.F.,.F.,0  
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Уволить с',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxBeg',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateUvol',.F.,IIF(par1,.T.,.F.),0 
     
 
     DO adTboxAsCont WITH 'frmOrd','ordDolNew',.ordPrik.Left,.ordBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'Перевести в',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordKse',.ordPrik.Left,.ordDolNew.Top+dHeight-1,.ordPrik.Width,dHeight,'Ссылка',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxLink',.ordKse.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newsLink',.F.,.F.,0  
     
     DO adTboxAsCont WITH 'frmOrd','ordKomp',.ordPrik.Left,.ordKse.Top+dHeight-1,.ordPrik.Width,dHeight,'Компенсация',1,1  
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPerBeg',.tBoxKomp.Left+.tBoxKomp.Width-1,.ordKomp.Top,.tBoxKomp.Width,dHeight,'период с',1,1  
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPerEnd',.tBoxPerBeg.Left+.tBoxPerBeg.Width-1,.ordKomp.Top,.tBoxKomp.Width,dHeight,'период по',1,1  
     ...
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,.ordKomp.Top+dHeight-1,.ordPrik.Width,dHeight,'основание',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0       
...
DO CASE
   CASE nuvol=1
        REPLACE txtprn WITH 'Уволить '+cOrdFio+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', '+strDateUvol+' в связи с истечением срока трудового договора, '+ALLTRIM(newSlink)+CHR(13)+;
                'Бухгалтерии выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+'.'+ CHR(13)+CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)        
   CASE nuvol=2
        REPLACE txtprn WITH 'Уволить '+cOrdFio+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', '+strDateUvol+' по желанию работника, '+ALLTRIM(newSlink)+CHR(13)+;
                'Бухгалтерии выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+'.'+ CHR(13)+CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)        
   CASE nuvol=3
        REPLACE txtprn WITH 'Уволить '+cOrdFio+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', '+strDateUvol+' за прогулы без уважительных причин '+ALLTRIM(newSlink)+CHR(13)+;
                'Бухгалтерии выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+'.'+CHR(13)+CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)        
   CASE nuvol=4
        REPLACE txtprn WITH 'Уволить '+cOrdFio+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', '+strDateUvol+' в связи расторжением контракта по соглашению сторон, '+ALLTRIM(newSlink)+CHR(13)+;
                'Бухгалтерии выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+'.'+CHR(13)+CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)        
   CASE nuvol=5                 
        REPLACE txtprn WITH 'Уволить '+cOrdFio+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', '+strDateUvol+' в порядке перевода в '+ALLTRIM(newPlace)+','+ALLTRIM(newSlink)+CHR(13)+;
                'Бухгалтерии выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+CHR(13)+CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)       
   CASE nuvol=6        && по сосотоянию здоровья         
        REPLACE txtprn WITH 'Уволить '+cOrdFio+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', признанную МРЭК  с работы '+strDateUvol+;
                ' в связи с несоответствием занимаемой должности вследствие состояния здоровья, препятствующего продолжению данной работы,'+ALLTRIM(newSlink)+CHR(13)+;
                'с выплатой выходного пособия в размере двухнедельнего среднего заработка.'+CHR(13)+;
                'Выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+'.'+CHR(13)+CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)                                   
   CASE nuvol=7        && в связи со смертью            
        REPLACE txtprn WITH 'Прекратить контракт с '+cOrdFio+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', '+strDateUvol+' в вязи со смертью работника, '+ALLTRIM(newSlink)+CHR(13)+;
                'Выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+ CHR(13)+CHR(13)+'Основание: '+newOsnov+CHR(13)   
   CASE nuvol=8        && по совместительству
        REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+cOrdFio+' '+strDateUvol+' уволить с работы по совместительству на 0,5 штатной единицы (оформленному на время ), в связи с истечением срока трудового договора, '+ALLTRIM(newSlink)+CHR(13)+;
                'Бухгалтерии выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+'.'+ CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)                       
   CASE nuvol=9
        REPLACE txtprn WITH 'Уволить '+cOrdFio+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', '+strDateUvol+' в связи с призывом на военную службу, '+ALLTRIM(newSlink)+CHR(13)+;
                'Бухгалтерии выплатить:'+CHR(13)+;
                '1. Денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                strPerBeg+' по '+strPerEnd+'.'+CHR(13)+;
                '2. Выходное пособие в размере двухнедельного среднего заработка, в соответствии со статьёй 48 Трудового кодекса Республики Беларусь.'+CHR(13)+CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)                                                
   CASE nuvol=10   
        REPLACE txtprn WITH 'Уволить '+cOrdFio+', с работы по совместительству на 0,5 должности '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' '+strDateUvol+' в связи с приёмом на работу работника, для которого эта работа будет являться основной, '+ALLTRIM(newSlink)+CHR(13)+;    
                'Бухгалтерии выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+'.'+ CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)    
   CASE nuvol=11   
        REPLACE txtprn WITH 'Уволить '+cOrdFio+', '+strDateUvol+', с работы по совместительству на 0,5 штатные единицы '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' '+strDateUvol+' в связи с тем, работа по совместительству стала для работника основной, '+ALLTRIM(newSlink)+CHR(13)+;    
                'Бухгалтерии выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+'.'+ CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)                  
   CASE nuvol=12
        REPLACE txtprn WITH 'Уволить '+cOrdFio+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', '+strDateUvol+;
                 ' в связи со вступлением в законную силу приговора суда, которым работник осужден к наказанию, исключающему продолжение работы, '+ALLTRIM(newSlink)+CHR(13)+;
                'Бухгалтерии произвести окончательный расчёт.'+CHR(13)+'Основание: '+newOsnov  
   CASE nuvol=13
        REPLACE txtprn WITH 'Уволить '+cOrdFio+' '+strDateUvol+', с работы по совместительству на 0,5 должности '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' по желанию работника, '+ALLTRIM(newSlink)+CHR(13)+;    
                'Бухгалтерии выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+'.'+ CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)      
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDol',.ordPrik.Left,.ordPodr.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность',1,1 
     DO adTboxNew WITH 'frmOrd','tBoxDolj',.ordDol.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNDolj',.F.,.F.,0           
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Перевести с',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxBeg',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateBeg',.F.,IIF(par1,.T.,.F.),0,.F.   
     DO CASE
        CASE logPerevod=1            
             DO adTboxAsCont WITH 'frmOrd','ordFioNew',.ordPrik.Left,.ordBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'ФИО (кого замещают)',1,1      
             DO adTboxNew WITH 'frmOrd','tBoxFioNew',.ordFioNew.Top,.comboBox1.Left,.comboBox1.Width-RetTxtWidth('w...')-2,dHeight,'newFioZam',.F.,IIF(par1,.T.,.F.),0      
             ...
             .butKlntNew.Enabled=IIF(par1,.T.,.F.)          
             DO adTboxAsCont WITH 'frmOrd','ordPodrNew',.ordPrik.Left,.ordFioNew.Top+dHeight-1,.ordPrik.Width,dHeight,'Подразделение (где)',1,1 
             DO adTboxNew WITH 'frmOrd','tBoxPodrNew',.ordPodrNew.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newSPodr',.F.,IIF(par1,.T.,.F.),0           
             DO adTboxAsCont WITH 'frmOrd','ordDolNew',.ordPrik.Left,.ordPodrNew.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность (кого)',1,1 
             DO adTboxNew WITH 'frmOrd','tBoxDoljNew',.ordDolNew.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newSDolj',.F.,IIF(par1,.T.,.F.),0 
             DO adTboxAsCont WITH 'frmOrd','ordKse',.ordPrik.Left,.ordDolNew.Top+dHeight-1,.ordPrik.Width,dHeight,'Объём',1,1  
        ...
        CASE logPerevod=2         
             DO adTboxAsCont WITH 'frmOrd','ordPodrNew',.ordPrik.Left,.ordBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'Подразделение (куда)',1,1 
             ...
             DO adTboxAsCont WITH 'frmOrd','ordDolNew',.ordPrik.Left,.ordPodrNew.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность (куда)',1,1 
            ...
            .tBoxTr.Visible=IIF(par1,.F.,.T.)  
             ...
             DO adTboxAsCont WITH 'frmOrd','ordNKont',.ordPrik.Left,.ordTr.Top+dHeight-1,.ordPrik.Width,dHeight,'контракт №',1,1   
             DO adTboxNew WITH 'frmOrd','tBoxNKont',.ordNKont.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNKont',.F.,IIF(par1,.T.,.F.),0      
             DO adTboxAsCont WITH 'frmOrd','ordDKont',.ordPrik.Left,.ordNKont.Top,RetTxtWidth('wдата контрактаw'),dHeight,'дата контракта',1,1   
             DO adTboxNew WITH 'frmOrd','tBoxDKont',.ordNKont.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newdKont',.F.,IIF(par1,.T.,.F.),0  
             .tBoxNKont.Width=(.comboBox1.Width-.ordDKont.Width)/2
             ...
             .tBoxDKont.Left=.ordDKont.Left+.ordDKont.Width-1
             .tBoxDKont.Width=.comboBox1.Width-.tBoxNKont.Width-.ordDKont.Width+2      
             DO adTBoxAsCont WITH 'frmOrd','ordKomp',.ordPrik.Left,.ordNKont.Top+dHeight-1,.ordPrik.Width,dHeight,'На срок',1,1                                             
             ...
             DO adTboxAsCont WITH 'frmOrd','ordDol2',.ordPrik.Left,.ordKomp.Top+dHeight-1,.ordPrik.Width,dHeight,'заключить с',1,1   
             DO adTboxNew WITH 'frmOrd','tBoxDolj2',.ordDol2.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateBeg',.F.,IIF(par1,.T.,.F.),0      
             DO adTboxAsCont WITH 'frmOrd','ordDol3',.ordPrik.Left,.ordDol2.Top,RetTxtWidth('поw'),dHeight,'по',1,1   
             ...
             DO adTboxAsCont WITH 'frmOrd','ordPkont',.ordPrik.Left,.ordDol2.Top+dHeight-1,.ordPrik.Width,dHeight,'% за контракт',1,1  
             DO adTboxNew WITH 'frmOrd','tBoxKont',.ordPkont.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newPkont','Z',IIF(par1,.T.,.F.),0,'999'     
             .tBoxKont.Alignment=0
     ...
     ENDCASE  
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,IIF(logPerevod=1,.ordKse.Top,.ordDay.Top)+dHeight-1,.ordPrik.Width,dHeight,'основание',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0  
...
SELECT txtOrder
DO CASE 
   CASE logperevod=1 && на время декретного
        REPLACE txtprn WITH 'Перевести '+ALLTRIM(cOrdFio)+' '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', c '+strDateBeg+' '+LOWER(ALLTRIM(newSDolj))+' '+LOWER(ALLTRIM(newSPodr))+;
                ' на '+STR(newKse,4,2)+' должности, с оплатой согласно штатному расписанию, на время отсутствия основного работника '+ALLTRIM(newFioZam)+;
                ' находящейся в социальном отпуске "по уходу за ребенком до достижения им возраста 3-х лет".'+CHR(13)+CHR(13)+;
                'Основание:'+newOsnov+CHR(13)+' С приказом '+dim_agree(kodsex)
   CASE logperevod=2  && на время отсутствия с звключением контракта 
        strDateEnd=IIF(!EMPTY(newDateEnd),dateToString('newDateEnd',.T.),'')  
        REPLACE txtprn WITH 'Перевести '+ALLTRIM(cOrdFio)+' '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', работающую по трудовому договору, заключенному на время отсутствия основного работника '+;
                ' на вакантную должость cоплатой согласно штатному расписанию и заключить с ней контракт сроком на '+ALLTRIM(strSrok)+' с '+;
                 strDateBeg+' по '+strDateEnd+', установив дополнительные меры стимулирования:'+CHR(13)+;
                '     Надбавка за работу на условиях контракта - '+LTRIM(STR(newPkont))+' %;'+CHR(13)+;
                '     Предоставление дополнительного поощрительного трудового отпуска - '+LTRIM(STR(newDDop))+' календарный день.'+CHR(13)+CHR(13)+;
                'Основание:'+newOsnov+CHR(13)+' С приказом '+dim_agree(kodsex)
...
WITH frmOrd 
     ...
     DO CASE   
        CASE logPerevod=3
             DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,.ordKse.Top+dHeight-1,.ordPrik.Width,dHeight,'основание',1,1  
        CASE logPerevod=4             
             DO adTboxAsCont WITH 'frmOrd','ordPkont',.ordPrik.Left,.ordKse.Top+dHeight-1,.ordPrik.Width,dHeight,'% за контракт',1,1  
             DO adTboxNew WITH 'frmOrd','tBoxKont',.ordPkont.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newPkont','Z',IIF(par1,.T.,.F.),0,'999'     
             .tBoxKont.Alignment=0
...
WITH frmOrd 
     ...
     DO adTBoxAsCont WITH 'frmOrd','ordTr',.ordPodr.Left,.ordBeg.Top+dHeight-1,.comboBox1.Width,dHeight,'тип работы',2,1                                             
  ...
  *******2
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPodr2',.ordPrik.Left,.ordTr.Top+dHeight-1,.ordPrik.Width,dHeight,'Подразделение (куда)',1,1 
     DO addcombomy WITH 'frmOrd',22,.comboBox1.Left,.ordPodr2.Top,dHeight,.comboBox1.Width,IIF(par1,.T.,.F.),'newNPodr2','ALLTRIM(curSprPodr.name)',6,'','DO procPriemPodrMany WITH 2',.F.,.T.  
     .comboBox22.Visible=IIF(par1,.T.,.F.)
     .comboBox22.DisplayCount=17
     DO adTboxNew WITH 'frmOrd','tBoxPodr2',.ordPodr2.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNpodr2',.F.,.F.,0  
...
DO CASE 
   ...
   CASE logperevod=3 && на другую работу
        REPLACE txtprn WITH 'Перевести '+cOrdFio+' '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', c '+strDateBeg+' '+LOWER(cDolzam)+' '+LOWER(cPodrZam)+;
               ', с оплатой согласно штатному расписанию.'+CHR(13)+CHR(13)+'Основание:'+newOsnov+CHR(13)+' С приказом '+dim_agree(kodsex)
   CASE logperevod=4  && на условиях контракта 
        strDateEnd=IIF(!EMPTY(newDateEnd),dateToString('newDateEnd',.T.),'')  
        REPLACE txtprn WITH 'Перевести '+cOrdFio+' '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', с '+;
                 strDateBeg+' на условиях контратка по '+strDateEnd+' '+LOWER(cDolZam)+' '+LOWER(cPodrZam)+', с оплатой согласно штатному расписанию, установив дополнительные меры стимулирования:'+CHR(13)+;
                '     Надбавка за работу на условиях контракта - '+LTRIM(STR(newPkont))+' %;'+CHR(13)+;
                '     Предоставление дополнительного поощрительного трудового отпуска - '+LTRIM(STR(newDDop))+' календарный день.'+CHR(13)+CHR(13)+;
                'Основание:'+newOsnov+CHR(13)+CHR(13)+' С приказом '+dim_agree(kodsex) 
   CASE logPerevod=10   
        strmany=' (из них '      
        IF newKse2#0
           strmany=strmany+STR(newKse2,4,2)+' должности '+LOWER(ALLTRIM(newNDolj2))+' '+LOWER(ALLTRIM(newNPodr2))+' '
        ENDIF
        IF newKse3#0
           strmany=strmany+STR(newKse3,4,2)+' должности '+LOWER(ALLTRIM(newNDolj3))+' '+LOWER(ALLTRIM(newNPodr3))+' '
        ENDIF
        IF newKse4#0
           strmany=strmany+STR(newKse4,4,2)+' должности '+LOWER(ALLTRIM(newNDolj4))+' '+LOWER(ALLTRIM(newNPodr4))
        ENDIF
        strmany=strmany+')'
        SELECT txtOrder
        REPLACE txtprn WITH 'Перевести '+cOrdFio+' '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', с '+;
                strDateBeg+' на условиях контратка по '+strDateEnd+' '+ALLTRIM(LOWER(newNDolj2))+' '+ALLTRIM(LOWER(newNpodr2))+strMany+', с оплатой согласно штатному расписанию, установив дополнительные меры стимулирования:'+CHR(13)+;       
                '     Надбавка за работу на условиях контракта - '+LTRIM(STR(newPkont))+' %;'+CHR(13)+;
                '     Предоставление дополнительного поощрительного трудового отпуска - '+LTRIM(STR(newDDop))+' календарный день.'+CHR(13)+CHR(13)+;
                '     С приказом '+dim_agree(kodsex)+CHR(13)+;
                '     С правилами внутреннего трудового распорядка и '+CHR(13)+;
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDol',.ordPrik.Left,.ordPodr.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность',1,1 
     DO adTboxNew WITH 'frmOrd','tBoxDolj',.ordDol.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNDolj',.F.,.F.,0  
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Установить с',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxBeg',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateBeg',.F.,IIF(par1,.T.,.F.),0 
     DO adTboxAsCont WITH 'frmOrd','ordKse',.ordPrik.Left,.ordBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'Объём',1,1  
     DO addSpinnerMy WITH 'frmOrd','spinKse',.comboBox1.Left,.ordKse.Top,dheight,RetTxtWidth('9999999999'),'newKse',0.25,.F.,0,1.5           
     .spinKse.Enabled=IIF(par1,.T.,.F.) 
     DO adTBoxAsCont WITH 'frmOrd','ordTr',.spinKse.Left+.spinKse.Width-1,.ordKse.Top,RetTxtWidth('wтипw'),dHeight,'тип',2,1                                             
     DO addComboMy WITH 'frmOrd',11,.ordTr.Left+.ordTr.Width-1,.ordTr.Top,dheight,.comboBox1.Width-.spinKse.Width-.ordTr.Width+2,IIF(par1,.T.,.F.),'strType','curSprType.name',6,.F.,'newTr=curSprType.kod',.F.,.T. 
...
DO CASE
   CASE nuvol=1
        REPLACE txtprn WITH 'Установить '+cOrdFio+' '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', с '+strDateBeg+;
                ' полное рабочее время c оплатой согласно штатному расписанию. '+CHR(13)+CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)        
   CASE nuvol=2
         REPLACE txtprn WITH 'Установить '+cOrdFio+' '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', с '+strDateBeg+;
                ' неполное рабочее время '+' из расчёта '+LTRIM(STR(newKse,4,2)) +' должности, c оплатой согласно штатному расписанию. '+CHR(13)+CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)      
   CASE nuvol=3
        REPLACE txtprn WITH 'Уволить '+cOrdFio+' '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' '+strDateUvol+' за прогулы без уважительных причин '+ALLTRIM(newSlink)+CHR(13)+;
                'Бухгалтерии выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+ CHR(13)+CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)        
   ...
   CASE nuvol=7        && в связи со смертью     
        ...
        REPLACE txtprn WITH 'Прекратить контракт с '+ordFio+' '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+', '+strDateUvol+' в сязи со смертью работника, '+ALLTRIM(newSlink)+', в связи со смертью рпаботника.'+CHR(13)+;
                'Выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+ CHR(13)+CHR(13)+'Основание: '+newOsnov+CHR(13)                            
ENDCASE        
*************************************************************************************************************************        
...
IF logApp     
   SELECT datJob
   DO CASE
      CASE INLIST(curPeopOrder.supOrd,1,20,21,22,23)    && прием на условиях контракта, трудового договора
           ...
           ENDIF
           DO repnadjob
      CASE curPeopOrder.supOrd=2    && прием внешнего совместительства 
           IF SEEK(curPeopOrder.kodpeop,'people',1)
              SELECT people
           ...
           ENDIF
           DO repnadjob
      CASE curPeopOrder.supOrd=3      &&перевод с заключением контракта        
      ...
      CASE curPeopOrder.supOrd=4      && перевод на время декретной  
           SELECT datjob
           SET ORDER TO 7
           SEEK oldNidJob
           REPLACE dateOut WITH newDateBeg,dordout WITH dateOrdNew,nordout WITH LTRIM(STR(numOrdNew))+'-'+strOrdNew,kordOut WITH newKodOrder,nidout WITH curpeoporder.nid                 
      ...
      CASE curpeopOrder.supOrd=8   && прием на условиях контракта множественный
           IF SEEK(curPeopOrder.kodpeop,'people',1)
              SELECT people 
      ...
      CASE curpeopOrder.supOrd=9 
           ...
           ENDIF 
           DO repnadjob   
      CASE curPeopOrder.supOrd=33  &&перевод множественный
           IF SEEK(curPeopOrder.kodpeop,'people',1) &&перевод на условиях контракта
              SELECT people
           ...
           IF curpeoporder.kp4#0.AND.curpeoporder.kd4#0
              ...
              ENDIF 
           ENDIF            
      CASE INLIST(curPeopOrder.supOrd,31,32)      && перевод 
           IF SEEK(curPeopOrder.kodpeop,'people',1).AND.curPeopOrder.supOrd=32  &&перевод на условиях контракта
              SELECT people
              REPLACE pkont WITH curpeoporder.pkont,daykont WITH curPeopOrder.ddop           
           ENDIF 
           ...
           DO repnadjob                
           REPLACE curpeoporder.nidjob WITH datjob.nid,peoporder.nidjob WITH datjob.nid            
      CASE INLIST(curPeopOrder.supOrd,10,41,42,43,44,45,46,47,48,50,70)     && увольнение
           SELECT datJob
           oldOrdJob=SYS(21)
           SET ORDER TO 1
           SEEK curPeopOrder.kodpeop          
      ...
      CASE INLIST(curPeopOrder.supOrd,49)  
           ...
           SET ORDER TO &oldOrdJob           
           SELECT people   
      CASE INLIST(curPeopOrder.supOrd,17,18,19)    && Заключение и продление конракта
           SELECT people
           oldPeopOrd=SYS(21)
           SET ORDER TO 1
           SEEK curPeopOrder.kodpeop
...
ENDIF 
logPer=parper 
SELECT kod,name,otm,fl FROM sprtot WHERE sprtot.kspr=23 INTO CURSOR curSuplOrd ORDER BY kod   && курсор для дополнительных плюшек по отпуску
parPadejDol=2
parPadejFio=2
DO newNidOrder
DO removeObjectKadr 
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDol',.ordPrik.Left,.ordPodr.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность',1,1 
     DO adTboxNew WITH 'frmOrd','tBoxDolj',.ordDol.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNDolj',.F.,.F.,0   
     DO adTboxAsCont WITH 'frmOrd','ordDayOtp',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Кол-во дней',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDDop',.tBoxDayOtp.Left+.tBoxDayOtp.Width-1,.ordDayOtp.Top,(.comboBox1.Width-.tBoxDayOtp.Width*3+4)/2,dHeight,'доп.дней',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPKont',.tBoxDDop.Left+.tBoxDDop.Width-1,.ordDayOtp.Top,.ordDDop.Width,dHeight,'ненорм.',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDayOtp.Top+dHeight-1,.ordPrik.Width,dHeight,'Предоставить с',1,1   
     ...
     IF newKodPrik=51  && Для смещения периода
        DO adTboxAsCont WITH 'frmOrd','orddol2',.ordPrik.Left,boxTop,.ordPrik.Width,dHeight,'Сместить рабочий год дни ',1,1   
        DO adTboxNew WITH 'frmOrd','tBoxDolj2',boxTop,.comboBox1.Left,.tBoxDayOtp.Width,dHeight,'dayCost',.F.,IIF(par1,.T.,.F.),0,.F.        
        DO adTboxAsCont WITH 'frmOrd','orddol3',.tBoxDolj2.Left+.tBoxDolj2.Width-1,boxTop,RetTxtWidth('wнаw'),dHeight,'с',1,1   
        ...
        DO adTboxAsCont WITH 'frmOrd','orddol4',.tBoxDolj3.Left+.tBoxDolj3.Width-1,boxTop,.ordDol3.Width,dHeight,'на',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPerBeg',.ordPrik.Left,boxTop,.ordPrik.Width,dHeight,'За период с',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPerEnd',.ordPrik.Left,.ordPerBeg.Top,RetTxtWidth('поw'),dHeight,'по',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordSupl',.ordPrik.Left,.ordPerBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'Произвести',1,1          
     ...
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,.ordSupl.Top+dHeight-1,.ordPrik.Width,dHeight,'Основание',1,1            
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0  
...
*parFio Переменная для ФИО
*parDol Переменная для должности
*parPodr Переменная для подразделения
*padFio Падеж фамилии
*padDol Падеж должности
...
SELECT txtOrder
********************************************************************************************************************************
*                         Формирование текстовой части приказа о трудовом отпуске
********************************************************************************************************************************
...
IF newKodPrik=51.AND.dayCost#0
   REPLACE txtprn WITH UPPER(LEFT(ALLTRIM(cOrdDol),1))+LOWER(SUBSTR(ALLTRIM(cOrdDol),2))+' '+LOWER(cOrdPodr)+' '+cOrdFio+':'+CHR(13)+;
           '1.'+'Сместить рабочий год на '+LTRIM(STR(dayCost))+' календарных дней (c '+DTOC(newPbegOld)+' г. на '+DTOC(newPbegNew)+;
           ' г.) в связи с предоставлением в течение рабочего года отпуска без сохранения заработной платы в размере превышающем 14 календарных дней.'+CHR(13)+;
           '2. Предоставить трудовой отпуск на '+ALLTRIM(STR(newDayOtp))+IIF(newDayOtp>4,' календарных дней',IIF(newDayOtp=1,' календарный день',' календарных дня'))

    IF !EMPTY(newDDop)
       REPLACE txtprn WITH txtprn+' (из них '+ALLTRIM(STR(newDDop))+IIF(newDDop>4,' календарных дней',IIF(newDDop=1,' календарный день ',' календарных дня '))+' за контракт'+IIF(newDNorm#0,',',')')                     
    ENDIF
    IF !EMPTY(newDNorm)
       REPLACE txtprn WITH txtprn+IIF(newDDop=0,' (из них ','') +ALLTRIM(STR(newDNorm))+IIF(newDNorm>4,' календарных дней',IIF(newDNorm=1,' календарный день за ненормированный рабочий день)',' календарных дня за ненормированный рабочий день)'))
    ENDIF
        
    REPLACE txtPrn WITH txtPrn+' c '+strDateBeg+' по '+strDateEnd+' за рабочий период c '+strPerBeg+' по '+strPerEnd+CHR(13)+;
           IIF(EMPTY(newOrdSupl),CHR(13)+'Основание: '+newOsnov,'3.'+ALLTRIM(newOrdSupl)+'.'+CHR(13)+'Основание: '+newOsnov) 
ELSE
    REPLACE txtprn WITH UPPER(LEFT(ALLTRIM(cOrdDol),1))+LOWER(SUBSTR(ALLTRIM(cOrdDol),2))+' '+LOWER(cOrdPodr)+' '+ALLTRIM(cOrdFio)+IIF(EMPTY(newOrdSupl),'',':'+CHR(13)+'1.')+IIF(logPer,'Предоставить трудовой отпуск на ','Предоставить часть трудового отпуска на ')+ALLTRIM(STR(newDayOtp))+;
            IIF(newDayOtp>4,' календарных дней',IIF(newDayOtp=1,' календарный день',' календарных дня'))
    IF !EMPTY(newDDop)
       REPLACE txtprn WITH txtprn+' (из них '+ALLTRIM(STR(newDDop))+IIF(newDDop>4,' календарных дней',IIF(newDDop=1,' календарный день ',' календарных дня '))+' за контракт'+IIF(newDNorm#0,',',')')                     
    ENDIF
    IF !EMPTY(newDNorm)
    ...
    REPLACE txtPrn WITH txtPrn+' c '+strDateBeg+' по '+strDateEnd+' за рабочий период c '+strPerBeg+' по '+strPerEnd+CHR(13)+;
           IIF(EMPTY(newOrdSupl),CHR(13)+'Основание: '+newOsnov,'2.'+ALLTRIM(newOrdSupl)+'.'+CHR(13)+'Основание: '+newOsnov) 
ENDIF         
********************************************************************************************************************************
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDayOtp.Top+dHeight-1,.ordPrik.Width,dHeight,IIF(nuvol=1,'Предоставить с','Продлить с'),1,1   
     DO adTboxNew WITH 'frmOrd','tBoxBeg',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateBeg',.F.,IIF(par1,.T.,.F.),0,.F.,;
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPerBeg',.ordPrik.Left,.ordBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'Причина',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,.ordPerBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'Основание',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0       
...
KEYBOARD '{TAB}'
********************************************************************************************************************************
*                         Формирование текстовой части приказа о социальном отпуске
********************************************************************************************************************************
...
DO CASE
   CASE nuvol=1
        REPLACE txtprn WITH  UPPER(LEFT(ALLTRIM(cOrdDol),1))+LOWER(SUBSTR(ALLTRIM(cOrdDol),2))+' '+LOWER(cOrdPodr)+' '+ALLTRIM(cOrdFio)+' предоставить социальный отпуск без сохранения заработной платы на '+ALLTRIM(STR(newDayOtp))+' календарный день '+;
                ' c '+strDateBeg+' по '+strDateEnd+' '+ALLTRIM(newStrPrich)+'.'+CHR(13)+CHR(13)+'Основание: '+ newOsnov         
   CASE nuvol=2
        REPLACE txtprn WITH  UPPER(LEFT(ALLTRIM(cOrdDol),1))+LOWER(SUBSTR(ALLTRIM(cOrdDol ),2))+' '+LOWER(cordPodr)+' '+ALLTRIM(cordFio)+' продлить трудовой отпуск на '+ALLTRIM(STR(newDayOtp))+' календарный день '+;
                ' c '+strDateBeg+' по '+strDateEnd+' '+ALLTRIM(newStrPrich)+'.'+CHR(13)+CHR(13)+'Основание: '+ newOsnov                         
ENDCASE        
********************************************************************************************************************************
...
DO saveotpOrder               
********************************************************************************************************************************
*                                   День матери, донорский, до 3-лет, по беременности и родам
********************************************************************************************************************************
PROCEDURE procotpsup
PARAMETERS par1,par2
vSup=par2
** vSup=1 - день матери
** vSup=2,7 - донорский день
** vSup=3 - по беременности и родам
** vSup=4 - по уходу за ребенком до 3 лет
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDayOtp.Top+dHeight-1,.ordPrik.Width,dHeight,IIF(INLIST(vSup,5,10),'Прервать с','Предоставить с'),1,1   
     ...
     .tBoxEnd.Left=.ordEnd.Left+.ordEnd.Width-1
     .tBoxEnd.Width=.comboBox1.Width-.tBoxBeg.Width-.ordEnd.Width+2
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,.ordBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'основание',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0  
...
DO CASE
   CASE vSup=1
        REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(cOrdFio)+' предоставить один дополнительный свободный от работы день '+strDateBeg+;
        ' , как матери, воспитывающей детей в возрасте до 16 лет, с оплатой в размере среднего дневного  заработка.'+CHR(13)+CHR(13)+'Основание: '+newOsnov
   CASE vSup=2
        REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(cOrdFio)+', '+strdateBeg+' предоставить свободный от работы день в связи со сдачей крови и её компонентов.'+;
        CHR(13)+CHR(13)+'Основание: '+newOsnov
   CASE vSup=3
        REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(cOrdFio)+' c '+strDateBeg+' по '+strDateEnd+' предоставить социальный отпуск по беременности и родам.'+;
        CHR(13)+CHR(13)+'Основание: '+newOsnov
   CASE vSup=4
        REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(cOrdFio)+' предоставить социальный "отпуск по уходу за ребенком до достижения им возраста трёх лет"'+' c '+strDateBeg+' по '+strDateEnd+;
        CHR(13)+CHR(13)+'Основание: '+newOsnov
   CASE vSup=5
        REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(cOrdFio)+' c '+strDateBeg+' прервать социальный "отпуск по уходу за ребенком до достижения им возраста трёх лет", в связи с уходом в отпуск по беременности и родам.'+;
        CHR(13)+'Основание: '+newOsnov
   CASE vSup=6
        REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(cOrdFio)+' c '+strDateBeg+' прервать социальный "отпуск по уходу за ребенком до достижения им возраста трёх лет", в связи с уходом в отпуск по беременности и родам.'+;
        CHR(13)+'Основание: '+newOsnov     
    CASE vSup=7
        REPLACE txtprn WITH 'Освободить '+ALLTRIM(newFioPeop)+', '+LOWER(ALLTRIM(cOrdDol))+' '+LOWER(cOrdPodr)+', на 1 календарный день, '+strdateBeg+', от работы для выполнения донорской функции.'+;
        CHR(13)+CHR(13)+'Основание: '+newOsnov     
   CASE vSup=10
   
        REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(cOrdFio)+' c '+strDateBeg+' приступить к работе из социального "отпуска по уходу за ребенком до достижения им возраста трёх лет",с оплатой согласно штатному расписанию.'+;
        CHR(13)+'Основание: '+newOsnov
...
ELSE 
   ...
   REPLACE kodOtp WITH curpeopOrder.kodOtp,begOtp WITH curPeopOrder.dateBeg,endotp WITH curPeopOrder.dateEnd,kvoday WITH curPeopOrder.dayOtp,dayKont WITH curPeopOrder.ddop,;              
           ...
           nameotp WITH IIF(SEEK(kodotp,'curSprOtp',1),curSprOtp.name,''),osnov WITH 'пр.№ '+LTRIM(STR(datOrder.numOrd))+'-'+datOrder.strOrd+' от '+DTOC(datOrder.dateOrd)      
...
IF logNewRec.AND.logPer
   ...
   DELETE FOR kodotp>3   
   GO TOP
   LOCATE FOR !EMPTY(perEnd) && подстраховка для сведений со старой программы
   IF kodotp=1
      newPerBeg=IIF(!EMPTY(perEnd),perEnd+1,newPerBeg)
      newPerEnd=IIF(!EMPTY(newPerBeg),IIF(MOD(YEAR(newPerBeg),4)=0,newPerBeg+365-1,newPerBeg+365),newPerEnd)
   ELSE 
...
WITH frmOrd
     ...
     ENDIF 
     DO adTboxAsCont WITH 'frmOrd','ordTabn',.ordPrik.Left,.ordPrik.Top+.ordPrik.Height-1,.ordPrik.Width,dHeight,'Номер',1,1
     ...
     DO adTboxAsCont WITH 'frmOrd','ordFio',.ordPrik.Left,.ordTabn.Top+dHeight-1,.ordPrik.Width,dHeight,'Фамилия Имя Отчество',1,1      
...
*****************************************************************************************************************
*               форма для печати приказа
*****************************************************************************************************************
PROCEDURE formPrnOrder
fSupl=CREATEOBJECT('FORMSUPL')
logform=.F.
logKmMandat=.T. &&для печати командировочного удостоверения
logKmOrder=.T.  &&для печати приказа по командировкам
WITH fSupl
     .Caption='Печать приказа'
     DO adSetupPrnToForm WITH 20,20,400,.T.,.F.
     logWord=.T.
     
     IF newKodprik=201
        DO adCheckBox WITH 'fSupl','checkOrd','приказ',.Shape91.Top+.Shape91.Height+10,.Shape91.Left,150,dHeight,'logKmOrder',0           
        DO adCheckBox WITH 'fSupl','checkMandat','удостоверение',.checkOrd.Top,.Shape91.Left,150,dHeight,'logKmMandat',0           
        ...
        DO adCheckBox WITH 'fSupl','check1','создать новый файл Word',.checkOrd.Top+.checkOrd.Height+10,.Shape91.Left,150,dHeight,'logForm',0           
     ELSE 
        DO adCheckBox WITH 'fSupl','check1','создать новый файл Word',.Shape91.Top+.Shape91.Height+10,.Shape91.Left,150,dHeight,'logForm',0           
     ENDIF 
    .check1.Left=.Shape91.Left+(.Shape91.Width-.check1.Width)/2 
    
    
     DO addButtonOne WITH 'fSupl','butPrn',.Shape91.Left+(.Shape91.Width-RetTxtWidth('wпросмотрw')*3-20)/2,.check1.Top+.check1.Height+20,'печать','','DO prnOrder WITH .T.',39,RetTxtWidth('wпросмотрw'),'печать приказа' 
     DO addButtonOne WITH 'fSupl','butView',.butPrn.Left+.butPrn.Width+10,.butPrn.Top,'просмотр','','DO prnOrder WITH .F.',39,.butPrn.Width,'просмотр' 
     DO addButtonOne WITH 'fSupl','butRet',.butView.Left+.butView.Width+10,.butPrn.Top,'возврат','','fSupl.Release',39,.butPrn.Width,'возврат' 
...
fSupl.Show
*****************************************************************************************************************
*                           непосредственно печать приказа
*****************************************************************************************************************
PROCEDURE prnOrder
PARAMETERS par1
...
SELECT curpeoporder
IF par1
   DO procForPrintAndPreview WITH 'repOrder','приказ',.T.,'orderToWord'
ELSE 
   DO procForPrintAndPreview WITH 'repOrder','приказ',.F.
ENDIF 
******************************************************************************************************************
...
ELSE
   ...
   WITH docRef
        ...
        .LeftPara 
        .Insert('УЧРЕЖДЕНИЕ ЗДРАВООХРАНЕНИЯ')
        docRef.CloseParaBelow  &&Удаляем лишний интервал после абзаца 
        .Insert(cr)
        .Font('Times New Roman',14)
        .LeftPara 
        .Insert('БРЕСТСКАЯ ЦЕНТРАЛЬНАЯ')
         docRef.CloseParaBelow  &&Удаляем лишний интервал после абзац
        .Insert(cr)
        .Font('Times New Roman',14)
        .LeftPara 
        .Insert('ГОРОДСКАЯ БОЛЬНИЦА')
        docRef.CloseParaBelow  &&Удаляем лишний интервал после абзац
        .Insert(cr)     
        .Insert(cr)
        .Font('Times New Roman',14)
        ...
        WITH ordTable1
             ...
             docRef.CloseParaBelow  &&Удаляем лишний интервал после абзаца           
             .cell(1,2).Range.Text='г. Брест' 
             .cell(1,2).Range.Select
             docRef.RightPara  && выравниеваем слева
             docRef.Font('Times New Roman',14) 
        ...
        SCAN ALL
             ...
             .CloseParaAbove     
             .CloseParaBelow  &&Удаляем лишний интервал после абзаца 
             .Insert(cr)    
        ENDSCAN
        .Insert(cr)
        .Font('Times New Roman',14)
        ...
        .InsertNewPage &&вставляем новую страницу
        .Insert(cr)
        .Insert(cr)
        ...
        WITH ordTable2
             ...
             .cell(1,1).Range.Text='Должность'
             .Columns(2).Width=120  
             .Cell(1,2).Select
             docRef.CenterPara  
             docRef.Font('Times New Roman',12)   
             .cell(1,2).Range.Text='Подпись'        
             .Cell(1,3).Select
             docRef.CenterPara  
             docRef.Font('Times New Roman',12)   
             .cell(1,3).Range.Text='Ф.И.О.'
             .Columns(3).Width=120        
             .Cell(1,4).Select
             docRef.CenterPara  
             docRef.Font('Times New Roman',12)   
             .cell(1,4).Range.Text='Дата'        
             .Columns(4).Width=80
             ...
             docRef.CloseParaBelow  &&Удаляем лишний интервал после абзаца           
             .Rows.Add  
             .cell(2,1).Range.Text='Начальник ОК' 
             .cell(2,1).Select
             docRef.LeftPara 
             .cell(2,3).Range.Text='С.И.Шепетюк' 
             .cell(2,4).Select
             docRef.LeftPara         
             .Rows.Add  
             .cell(3,1).Range.Text='Ведущий юрисконсульт' 
             .cell(3,1).Range.Select
             .cell(3,3).Range.Text='Е.Н.Сегодник' 
...
IF logKmMandat 
   ...
   * Подставляем необходимые значения в поля  
   ...
   IF TYPE([nameDoc.formFields("cPrik")])="O"
      nameDoc.FormFields("cPrik").Result='"'+STR(DAY(curpeoporder.dord),2)+'" '+ALLTRIM(month_prn(MONTH(curpeoporder.dord)))+' '+STR(YEAR(curpeoporder.dord),4) +' г.  № '+ALLTRIM(curpeoporder.nord)
   ENDIF
...
ENDIF 
*************************************************************************************************************************
*                   Процедура деления ФИО на фамилию и инициалы
*************************************************************************************************************************
...
WITH frmOrd 
     DO addObjectOrder    
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordFio.Top+dHeight-1,.ordPrik.Width,dHeight,'Разрешить с',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxBeg',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateBeg',.F.,IIF(par1,.T.,.F.),0      
             
     DO adTboxAsCont WITH 'frmOrd','ordPodr',.ordPrik.Left,.ordBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'Подразделение (основн.)',1,1     
     DO adTboxNew WITH 'frmOrd','tBoxPodr',.ordPodr.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNPodr',.F.,IIF(par1,.T.,.F.),0  
       
     DO adTboxAsCont WITH 'frmOrd','ordDol',.ordPrik.Left,.ordPodr.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность (основн.)',1,1 
     DO adTboxNew WITH 'frmOrd','tBoxDolj',.ordDol.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNdolj',.F.,IIF(par1,.T.,.F.),0  
     
     
     DO adTboxAsCont WITH 'frmOrd','ordPodrNew',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Подразделение (совмест.)',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDolNew',.ordPrik.Left,.ordPodrNew.Top+dHeight-1,.ordPrik.Width,dHeight,'Должность (совмест.)',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,IIF(!logDek,.ordTr.Top+dHeight-1,.ordFioNew.Top+dHeight-1),.ordPrik.Width,dHeight,'основание',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0 
...
SELECT txtOrder
IF logPers
   REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(str_ini)+' с '+strDateBeg+;
           ' установить доплату в размере '+ALLTRIM(STR(newKse*100))+'% оклада за расширенную зону обслуживания по должности '+;
           LOWER(ALLTRIM(cDolZam))+' '+LOWER(ALLTRIM(cPodrZam))+' без освобождения от своей основной работы в течение установленной законодательством продолжительности рабочего дня'+;
           IIF(logDek,' на время отсутствия основного работника '+str_zam+', находящегося в социальном отпуске "по уходу за ребенком до достижения им возраста 3-х лет".','.')+CHR(13)+'Основание: '+ALLTRIM(newOsnov)        
ELSE 
   IF nuvol=2
           REPLACE txtprn WITH ALLTRIM(str_ini)+' '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' с '+strDateBeg+;
                   ' принять по совместительству на '+STR(newKse,4,2)+' штатные единицы '+LOWER(cDolZam)+' '+LOWER(cPodrZam)+;
                   ', с оплатой согласно штатному расписанию.'+CHR(13)+'Основание: '+ALLTRIM(newOsnov)      
   ELSE
      REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(str_ini)+' с '+strDateBeg+;
              ' разрешить работу сверх продолжительности рабочего времени по основной работе в пределах 900 часов в год на '+STR(newKse,4,2)+;
              ' должности '+LOWER(ALLTRIM(cDolZam))+' '+LOWER(ALLTRIM(cPodrZam))+' на основании графика работы, с оплатой за фактически отработанное время'+;
              IIF(logDek,' на время отсутствия основного работника '+str_zam+', находящегося в социальном отпуске "по уходу за ребенком до достижения им возраста 3-х лет"','.')+CHR(13)+'Основание: '+ALLTRIM(newOsnov)        
   ENDIF        
ENDIF         
********************************************************************************************************************************
...
KEYBOARD '{TAB}'
*************************************************************************************************************************
*                         Отмена внутреннего совместительства
*************************************************************************************************************************
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordFio',.ordPrik.Left,.ordPrik.Top+dHeight-1,.ordPrik.Width,dHeight,'Фамилия Имя Отчество',1,1      
     DO adTboxNew WITH 'frmOrd','tBoxFio',.ordFio.Top,.comboBox1.Left,.comboBox1.Width-RetTxtWidth('w...')-2,dHeight,'newFioPeop',.F.,IIF(par1,.T.,.F.),0      
      .tBoxFio.procforChange='DO changePeopOrd'   
     DO adtboxnew WITH 'frmOrd','boxFree',.tBoxFio.Top,.tBoxFio.Left+.tBoxFio.Width-1,.comboBox1.Width-.tBoxFio.Width+1,dheight,'',.F.,IIF(par1,.T.,.F.)   
     DO addButtonOne WITH 'frmOrd','butKlnt',.tBoxFio.Left+.tBoxFio.Width+1,.tBoxFio.Top+2,'','sbdn.ico','DO selectPeopOrd',.tBoxFio.Height-4,RetTxtWidth('w...')-1,'' 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordTabn',.ordPrik.Left,.ordFio.Top+dHeight-1,.ordPrik.Width,dHeight,'Номер',1,1
     DO adTboxNew WITH 'frmOrd','tBoxTabn',.ordTabn.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newKodPeop',.F.,IIF(par1,.T.,.F.),0,'99999','DO validOrderNum'  
     .tBoxTabn.Alignment=0
               
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordTabn.Top+dHeight-1,.ordPrik.Width,dHeight,IIF(nuvol=3,'Уволить с','Освободить с'),1,1   
    * DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordTabn.Top+dHeight-1,.ordPrik.Width,dHeight,'Освободить с',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxBeg',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateBeg',.F.,IIF(par1,.T.,.F.),0      
     ...
     IF nuvol=3
       ...
       * DO adTboxNew WITH 'frmOrd','tBoxLink',.ordKse.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newsLink',.F.,.F.,0  
        ...
        DO adTboxAsCont WITH 'frmOrd','ordKomp',.ordPrik.Left,.ordTr.Top+dHeight-1,.ordPrik.Width,dHeight,'Компенсация',1,1  
        DO adTboxNew WITH 'frmOrd','tBoxKomp',.ordKomp.Top,.comboBox1.Left,.comboBox1.Width/5,dHeight,'newDayKomp','Z',IIF(par1,.T.,.F.),0 
        .tBoxKomp.InputMask='99' 
        .tBoxKomp.Alignment=0
        DO adTboxAsCont WITH 'frmOrd','ordPerBeg',.tBoxKomp.Left+.tBoxKomp.Width-1,.ordKomp.Top,.tBoxKomp.Width,dHeight,'период с',1,1  
     ...
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,IIF(nuvol=3,.ordKomp.Top+dHeight-1,.ordTr.Top+dHeight-1),.ordPrik.Width,dHeight,'основание',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0 
...
SELECT txtOrder
IF nuvol=3
   REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(cOrdFio)+' '+strDateBeg+' уволить с работы по совместительству на '+LTRIM(STR(newkse,5,2))+' должности по желанию работника, '+ALLTRIM(newSlink)+CHR(13)+;
           'Бухгалтерии выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
                 strPerBeg+' по '+strPerEnd+'.'+ CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)                       
          
*   REPLACE txtprn WITH ALLTRIM(newNDolj)+' '+LOWER(ALLTRIM(newNPodr))+' '+ALLTRIM(newFioPeop)+' '+strDateBeg+' уволить с работы по совместительству на 0,5 штатной единицы (оформленному на время ), в связи с истечением срока трудового договора, '+ALLTRIM(newSlink)+CHR(13)+;
 *               'Бухгалтерии выплатить денежную компенсацию за '+LTRIM(STR(newDayKomp))+ IIF(newDayKomp>4,' календарных дней',IIF(newDayKomp=1,' календарный день',' календарных дня'))+' неиспользованного трудового отпуска за период работы с '+;
 *                strPerBeg+' по '+strPerEnd+'.'+ CHR(13)+'Основание: '+newOsnov+CHR(13)+'С приказом '+dim_agree(kodsex)             
ELSE 
   IF logPers
      REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(str_ini)+' с '+strDateBeg+;
              ' освободить от доплаты '+ALLTRIM(STR(newKse*100))+'% за совмещение по должности '+LOWER(cDolzam)+' '+LOWER(cPodrZam)+'.'
              REPLACE txtprn WITH ALLTRIM(txtprn)+CHR(13)+'Основание: '+ALLTRIM(newOsnov) 
   ELSE 
      REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(str_ini)+' с '+strDateBeg+;
              ' освободить от выполнения работы сверх продолжительности рабочего времени по основной работе на '+STR(newKse,4,2)+;
              ' ставки по должности '+LOWER(cDolZam)+' '+LOWER(cPodrZam)+'.'
              REPLACE txtprn WITH ALLTRIM(txtprn)+CHR(13)+'Основание: '+ALLTRIM(newOsnov)                     
   ENDIF         
ENDIF 
*************************************************************************************************************************
...
ENDIF
IF !USED('curPrichZam')
   SELECT kod,name FROM sprtot WHERE sprtot.kspr=30 INTO CURSOR curPrichZam  && курсор для причин заместительства
ENDIF 
IF !USED('curRMat')
   ...
   APPEND BLANK
   APPEND BLANK
   REPLACE name WITH 'с полной материальной ответственностью и подписанием акта приема-передачи'
 ENDIF
logTxt=par2
...
WITH frmOrd 
     ...
     DO CASE   
        CASE logtxt=1
             DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Установить с',1,1   
        CASE logtxt=2
             DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Разрешить с',1,1   
        CASE logtxt=3
             DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Перевести с',1,1   
        CASE logtxt=4
             DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Принять с',1,1                          
     ...
     .spinKse.Enabled=IIF(par1,.T.,.F.)
     IF par2=1
        DO adTboxAsCont WITH 'frmOrd','ordNKse',.spinKse.Left+.spinKse.Width-1,.ordKse.Top,RetTxtWidth('wед/должн.w'),dHeight,'ед/долж.',1,1
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPerBeg',.ordPrik.Left,.ordKse.Top+dHeight-1,.ordPrik.Width,dHeight,'Причина',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPodr4',.ordPrik.Left,.ordPerBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'Мат.ответственность',1,1 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,.ordPodr4.Top+dHeight-1,.ordPrik.Width,dHeight,'основание',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0  
...
DO CASE
   CASE logTxt=1
        REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+' '+ALLTRIM(str_ini)+' с '+strDateBeg+;
                ' разрешить работу сверх продолжительности рабочего времени по основной работе на '+STR(newKse,4,2)+IIF(EMPTY(nkse),' штатные единицы ',' '+ALLTRIM(nkse)+' ')+LOWER(cDolZam)+' '+LOWER(cPodrZam)+;
                ', с оплатой за фактически отработанное время, '+ALLTRIM(newStrPrich)+' '+str_zam+CHR(13)+'Основание: '+ALLTRIM(newOsnov) 
   CASE logTxt=2
        REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPOdr)+' '+ALLTRIM(str_ini)+' с '+strDateBeg+;
                ' установить доплату в размере '+LTRIM(STR(newKse*100))+'% оклада по должности '+LOWER(cDolZam)+' '+LOWER(cPodrZam)+;
                ' на период исполнения обязанностей временно отсутствующего работника в течение установленной законодательством продолжительности рабочего дня, '+;
                ALLTRIM(newStrPrich)+' '+str_zam+CHR(13)+'Основание: '+ALLTRIM(newOsnov)
   CASE logtxt=3
        REPLACE txtprn WITH ALLTRIM(str_ini)+' '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' с '+strDateBeg+;
                ' перевести'+IIF(!EMPTY(newNKurs),', '+ALLTRIM(newNKurs),'')+' на работу '+LOWER(cDolZam)+' '+LOWER(cPodrZam)+;
                ', с оплатой согласно штатному расписанию, '+ALLTRIM(newStrPrich)+' '+str_zam+CHR(13)+'Основание: '+ALLTRIM(newOsnov)                          
   CASE logtxt=4
        REPLACE txtprn WITH ALLTRIM(str_ini)+' '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+' с '+strDateBeg+;
                ' принять по совместительству на '+STR(newKse,4,2)+' штатные единицы '+LOWER(cDolZam)+' '+LOWER(cPodrZam)+;
                ', с оплатой согласно штатному расписанию, '+ALLTRIM(newStrPrich)+' '+str_zam+CHR(13)+'Основание: '+ALLTRIM(newOsnov)               
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Направить с',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxBeg',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateBeg',.F.,IIF(par1,.T.,.F.),0,.F.
     ...
     DO adTboxAsCont WITH 'frmOrd','ordFioNew',.ordPrik.Left,.ordBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'Учебное заведение',1,1      
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDolNew',.ordPrik.Left,.ordFioNew.Top+dHeight-1,.ordPrik.Width,dHeight,'Наименование курсов',1,1 
     DO adTboxNew WITH 'frmOrd','tBoxDoljNew',.ordDolNew.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNKurs',.F.,IIF(par1,.T.,.F.),0        
     
     DO adTboxAsCont WITH 'frmOrd','ordTr',.ordPrik.Left,.ordDolNew.Top+dHeight-1,.ordPrik.Width,dHeight,'Вид курсов',1,1          
     ...
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,.ordTr.Top+dHeight-1,.ordPrik.Width,dHeight,'основание',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0  
...
DO procfioini WITH 'cOrdFio' 
SELECT txtOrder
REPLACE txtprn WITH UPPER(LEFT(cOrdDol,1))+LOWER(SUBSTR(cOrdDol,2))+' '+LOWER(cOrdPodr)+' '+ALLTRIM(str_ini)+' с '+strDateBeg+' по '+strDateEnd+;
                ' направить в '+ALLTRIM(newNSchool)+' на курсы повышения квалификации по программе '+ALLTRIM(newNKurs)+' с сохранением средней заработной платы по месту работы.'+CHR(13)+'Основание: '+ALLTRIM(newOsnov) 
******************************************************************************************************************************************************        
...
IF logApp           
   SELECT datJob
   DO CASE
      CASE INLIST(curPeopOrder.supOrd,101,102,104,105,113)    && внутренне совместительство+расширенная зона обслуживания  в т.ч. вместо декретчиков (отображается в datjob)                                       
           ...
           ENDIF                  
           SELECT datJob                                                                    
      CASE INLIST(curPeopOrder.supOrd,106,107)    && отмена совместительства
           SELECT datjob
           SET ORDER TO 7
           SEEK curPeopOrder.oldNid
           ...
           *-----неполная отмена совместительства
           IF kserep#0          
              IF curPeopOrder.nidjob=0                 
                 SET DELETED OFF 
                 SET ORDER TO 7
              ...
              SELECT datjob 
           ENDIF
      CASE curPeopOrder.supOrd=115  &&увольнение по совместительству
           SELECT datjob
           SET ORDER TO 7
           SEEK curPeopOrder.oldnid         
           REPLACE dateOut WITH curPeopOrder.dateBeg,dordOut WITH dateOrdNew,nordout WITH LTRIM(STR(numOrdNew))+'-'+strOrdNew,kordOut WITH newKodOrder
      CASE curPeopOrder.supOrd=141    && Курсы
           SELECT datKurs
           SET ORDER TO 3
           SEEK curPeopOrder.nid
           IF !FOUND()
      ...
      CASE curpeopOrder.supOrd=170
           ...
           ENDIF  
      CASE curpeoporder.supOrd=180 && изменение фамилии
           IF SEEK(curpeoporder.kodpeop,'people',1) 
              SELECT people
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPkont',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Направить куда (город)',1,1         
     DO adTboxNew WITH 'frmOrd','tBoxKont',.ordPkont.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newCity',.F.,IIF(par1,.T.,.F.),0,
     DO adTboxAsCont WITH 'frmOrd','ordDayOtp',.ordPrik.Left,.ordPKont.Top+dHeight-1,.ordPrik.Width,dHeight,'кол-во дней',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxDayotp',.ordDayOtp.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDayOtp',.F.,IIF(par1,.T.,.F.),0,'99' 
     .tBoxDayOtp.Alignment=0
        
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDayOtp.Top+dHeight-1,.ordPrik.Width,dHeight,'Направить с',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordTr',.ordPrik.Left,.ordDolNew.Top+dHeight-1,.ordPrik.Width,dHeight,'Вид командировки',1,1          
     DO addComboMy WITH 'frmOrd',22,.comboBox1.Left,.ordTr.Top,dheight,.comboBox1.Width,IIF(par1,.T.,.F.),'newNpodr2','curTypeKurs.name',6,.F.,'newKp2=curTypeKurs.kod',.F.,.T. 
     .comboBox22.Visible=IIF(par1,.T.,.F.)
     DO adTboxNew WITH 'frmOrd','tBoxPodr2',.ordTr.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNpodr2',.F.,.F.,0  
     .tBoxPodr2.Visible=IIF(par1,.F.,.T.) 
...
SELECT txtOrder
DO CASE
   CASE newKp2=1 &&курсы плвышеня квалификации
        REPLACE txtprn WITH '1. Направить '+ALLTRIM(cOrdFio)+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+ ' в '+ALLTRIM(newCity)+;
                ' '+ALLTRIM(newNSchool)+' на '+LTRIM(STR(newDayOtp))+IIF(newDayOtp>4,' календарных дней',IIF(newDayOtp=1,' календарный день',' календарных дня'))+' с '+strDateBeg+' по '+strDateEnd+;
                ' на повышение квалификации по программе '+ALLTRIM(newNKurs)+' с сохранением средней заработной платы по месту работы.'+CHR(13)+;
                '2. Оплату командировочных расходов произвести в соответствии с действующим законодательством.'+CHR(13)+ CHR(13)+'Основание: '+ALLTRIM(newOsnov) 
   CASE newKp2=2 &&переподготовка
        REPLACE txtprn WITH '1. Направить '+ALLTRIM(cOrdFio)+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+ ' в '+ALLTRIM(newCity)+;
                ' '+ALLTRIM(newNSchool)+' на '+LTRIM(STR(newDayOtp))+IIF(newDayOtp>4,' календарных дней',IIF(newDayOtp=1,' календарный день',' календарных дня'))+' с '+strDateBeg+' по '+strDateEnd+;
                ' на переподготовку по программе '+ALLTRIM(newNKurs)+' с сохранением средней заработной платы по месту работы.'+CHR(13)+;
                '2. Оплату командировочных расходов произвести в соответствии с действующим законодательством.'+CHR(13)+ CHR(13)+'Основание: '+ALLTRIM(newOsnov)              
   CASE newKp2=5 &&клиническая ординатура
        REPLACE txtprn WITH '1. Направить '+ALLTRIM(cOrdFio)+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+ ' в '+ALLTRIM(newCity)+;
                ' '+ALLTRIM(newNSchool)+' на '+LTRIM(STR(newDayOtp))+IIF(newDayOtp>4,' календарных дней',IIF(newDayOtp=1,' календарный день',' календарных дня'))+' с '+strDateBeg+' по '+strDateEnd+;
                ' на повышение квалификации по программе '+ALLTRIM(newNKurs)+' с сохранением средней заработной платы по месту работы.'+CHR(13)+;
                '2. Оплату командировочных расходов произвести в соответствии с действующим законодательством.'+CHR(13)+ CHR(13)+'Основание: '+ALLTRIM(newOsnov) 
   CASE newKp2=6   &&сдача квалификационного экзамена   
        REPLACE txtprn WITH '1. Направить '+ALLTRIM(cOrdFio)+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+ ' в '+ALLTRIM(newCity)+;
                ' '+ALLTRIM(newNSchool)+' для сдачи квалификационного экзамена '+;
                ' на '+LTRIM(STR(newDayOtp))+IIF(newDayOtp>4,' календарных дней',IIF(newDayOtp=1,' календарный день ',' календарных дня '))+IIF(newDayOtp=1,strDateBeg,'с '+strDateBeg+' по '+strDateEnd)+;
                ' с сохранением средней заработной платы по месту работы.'+CHR(13)+;
                '2. Оплату командировочных расходов произвести в соответствии с действующим законодательством.'+CHR(13)+ CHR(13)+'Основание: '+ALLTRIM(newOsnov) 
   OTHERWISE 
        REPLACE txtprn WITH '1. Направить '+ALLTRIM(cOrdFio)+', '+LOWER(cOrdDol)+' '+LOWER(cOrdPodr)+ ' в '+ALLTRIM(newCity)+;
                ' '+ALLTRIM(newNSchool)+' на '+LTRIM(STR(newDayOtp))+IIF(newDayOtp>4,' календарных дней',IIF(newDayOtp=1,' календарный день',' календарных дня'))+' с '+strDateBeg+' по '+strDateEnd+;
...
DO CASE
   CASE curPeopOrder.supOrd=201
        IF INLIST(curpeoporder.kp2,1,2,3)    && Командировка на курсы -  командировки          
           SELECT datKurs
           SET ORDER TO 3
           SEEK curPeopOrder.nid
           IF !FOUND()
        ...
        ELSE              
           ...
           SELECT curPeopOrder
        ENDIF  
   *CASE curPeopOrder.supOrd=201    && Командировка на курсы - административные
   *     SELECT datKurs
   *     SET ORDER TO 3
   *     SEEK curPeopOrder.nid
   *     IF !FOUND()
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordPkont',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Новая фамилия',1,1         
     DO adTboxNew WITH 'frmOrd','tBoxKont',.ordPkont.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newFam',.F.,IIF(par1,.T.,.F.),0,
        
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordPkont.Top+dHeight-1,.ordPrik.Width,dHeight,'Изменить с',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxBeg',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateBeg',.F.,IIF(par1,.T.,.F.),0,.F.
...
SELECT txtOrder
REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+ ' '+ALLTRIM(str_ini)+' с '+strDateBeg+;
        'изменить в учётных и иных официальных документах фамилию '+ALLTRIM(oldFio_cx)+' на '+ALLTRIM(newFam)+' в связи с бракосочетанием' +CHR(13)+'Основание: '+ALLTRIM(newOsnov) 
*************************************************************************************************************************
...
DO removeObjectKadr
DO resetVarTot 
newNKont=IIF(logNewRec,SPACE(20),ALLTRIM(SUBSTR(curpeoporder.varsupl,1,20)))     && начало
newNschool=IIF(logNewRec,SPACE(20),ALLTRIM(SUBSTR(curpeoporder.varsupl,21,20)))  && обед
newNkurs=IIF(logNewRec,SPACE(20),ALLTRIM(SUBSTR(curpeoporder.varsupl,41,20)))    && окончание
newStrPrich=IIF(logNewRec,'по семейным обстоятельствам',ALLTRIM(SUBSTR(curpeoporder.varsupl,61,60)))    && причина
newLogApp=.T.
newNpp=0
str_ini=''
logDatJob=.F.
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordBeg',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordDol.Width,dHeight,'Изменить с',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxBeg',.ordBeg.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newDateBeg',.F.,IIF(par1,.T.,.F.),0,.F.
     
     DO adTboxAsCont WITH 'frmOrd','ordDay',.ordPrik.Left,.ordBeg.Top+dHeight-1,.ordPrik.Width,dHeight,'Причина',1,1         
     DO adTboxNew WITH 'frmOrd','tBoxday',.ordDay.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newStrPrich',.F.,IIF(par1,.T.,.F.),0,
     
     DO adTboxAsCont WITH 'frmOrd','ordPodr2',.ordPrik.Left,.ordDay.Top+dHeight-1,.ordPrik.Width,dHeight,'Начало рабочего времени',1,1         
     DO adTboxNew WITH 'frmOrd','tBoxPodr2',.ordPodr2.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNKont',.F.,IIF(par1,.T.,.F.),0,
     
     DO adTboxAsCont WITH 'frmOrd','ordPodr3',.ordPrik.Left,.ordPodr2.Top+dHeight-1,.ordPrik.Width,dHeight,'Обеденный перерыв',1,1         
     DO adTboxNew WITH 'frmOrd','tBoxPodr3',.ordPodr3.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newNSchool',.F.,IIF(par1,.T.,.F.),0,
     
     DO adTboxAsCont WITH 'frmOrd','ordPodr4',.ordPrik.Left,.ordPodr3.Top+dHeight-1,.ordPrik.Width,dHeight,'Окончание рабочего времени',1,1         
...
SELECT txtOrder
REPLACE txtprn WITH cOrdDol+' '+LOWER(cOrdPodr)+ ' '+ALLTRIM(str_ini)+' с '+strDateBeg+;
        'изменить график работы '+ALLTRIM(newStrPrich)+':'+CHR(13)+;
        'начало рабочего времени - '+ALLTRIM(newNkont)+CHR(13)+;
        'обеденный перерыв - '+ALLTRIM(newnSchool)+CHR(13)+;
        'окончание рабочего времени - '+ALLTRIM(newNKurs)+CHR(13)+'Основание: '+ALLTRIM(newOsnov)        
*************************************************************************************************************************
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,.ordFio.Top+dHeight-1,.ordPrik.Width,dHeight,'основание',1,1  
     .ordOsnov.Visible=.F.
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0  
     .tBoxOsnov.Visible=.F.
...
WITH frmOrd 
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDol2',.ordPrik.Left,.ordDol.Top+dHeight-1,.ordPrik.Width,dHeight,'Параграф',1,1         
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDol3',.ordPrik.Left,.ordDol2.Top+dHeight-1,.ordPrik.Width,dHeight,'Номер',1,1   
     ...
     DO adTboxAsCont WITH 'frmOrd','ordDol4',.ordPrik.Left,.ordDol3.Top+dHeight-1,.ordPrik.Width,dHeight,'Дата',1,1   
     DO adTboxNew WITH 'frmOrd','tBoxDolj4',.ordDol4.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newPerBeg',.F.,IIF(par1,.T.,.F.),0,.F.
                   
     DO adTboxAsCont WITH 'frmOrd','ordOsnov',.ordPrik.Left,.ordDol4.Top+dHeight-1,.ordPrik.Width,dHeight,'Основание',1,1  
     DO adTboxNew WITH 'frmOrd','tBoxOsnov',.ordOsnov.Top,.comboBox1.Left,.comboBox1.Width,dHeight,'newOsnov',.F.,IIF(par1,.T.,.F.),0  
...
fNewOrder=LTRIM(STR(newDDop))+'-О'
IF SEEK(STR(newnidPeop,5)+DTOS(newPerBeg)+fNewOrder+STR(newPkont,3),'peoporder',3)
   nidJobNew=peoporder.nid
   SELECT txtOrder
   REPLACE txtprn WITH 'Параграф '+LTRIM(STR(newPkont))+' приказа '+fNewOrder+' от '+DTOC(newPerBeg)+' г.: "'+ALLTRIM(peoporder.txtord)+'" отменить.'+CHR(13)+'Основание: '+ALLTRIM(newOsnov)        
ENDIF
********************************************************************************************
